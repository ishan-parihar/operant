...
```bash
GSETUP="python ${HERMES_HOME:-$HOME/.operant}/skills/productivity/google-workspace/scripts/setup.py"
```
...
`~/.operant/google_oauth_last_url.txt`.
...
- Token is stored at `~/.operant/google_token.json` and auto-refreshes.
- Pending OAuth session state/verifier are stored temporarily at `~/.operant/google_oauth_pending.json` until exchange completes.
- If `gws` is installed, `google_api.py` points it at the same `~/.operant/google_token.json` credentials file. Users do not need to run a separate `gws auth login` flow.
...
```bash
GAPI="python ${HERMES_HOME:-$HOME/.operant}/skills/productivity/google-workspace/scripts/google_api.py"
```
