...
4. Store the token in `${HERMES_HOME:-~/.operant}/.env` (or via `operant setup`):
   ```
...
- **Always use the `terminal` tool with `curl`.** Do NOT use `web_fetch` (it can't send auth headers) or the `browser` tool navigate command (needs UI auth and is slow).
- **`AIRTABLE_API_KEY` flows from `${HERMES_HOME:-~/.operant}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
