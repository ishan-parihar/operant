...
| List/filter by metadata (age, source, cost, tokens, workspace) | `operant sessions list` / `stats` via terminal |
| Rename | `operant sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `operant sessions archive <filters>` |
| Delete (destructive) | `operant sessions delete` / `operant sessions prune <filters>` |
| Export before deleting anything valuable | `operant sessions export --session-id <id> --format md` |
...
`operant sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `operant sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `operant sessions list`
