...
### TUI backend (spawned by `operant chat`)
...
debugpy.wait_for_client()
```
Start `operant chat`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
