...
metadata:
  operant:
    tags: [skills, authoring, operant, conventions, skill-md]
    related_skills: [plan, requesting-code-review]
---

# Authoring Operant Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.operant/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `skills/<category>/<name>/SKILL.md` or `optional-skills/<category>/<name>/SKILL.md` inside the operant repo — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.

In-repo skills must meet the repo's **hardline authoring standards** (see AGENTS.md, "Skill authoring standards" — that section is the source of truth; this skill is the operational walkthrough). Reviewers reject PRs that violate them, so meeting them up front is cheaper than a salvage pass later.

## When to Use

- User asks you to add a skill "in this branch / repo / commit"
- You're committing a reusable workflow that should ship with operant
...
- Don't use for: personal skills in `~/.operant/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `operant skills market install <name>` or `operant skills install <dir>`.
...
Pick the category by what the tool IS, not what it feels like (an AI-agent CLI goes in `autonomous-ai-agents/` even if it "feels productivity"). Confirm existing categories with `file_search(pattern='*', target='files', path='skills')` and don't invent new top-level categories casually.
...
author: Real Name (github-handle), Operant
license: MIT
...
- Credit the **human first**, then "Operant" as secondary collaborator: `Ben Barclay (benbarclay), Operant`.
- Never `author: Operant` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Ishan Parihar (ishan-parihar), Operant`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.operant/skills/`.
- Verify each entry: `file_search(pattern='<name>', target='files', path='skills')` (and `optional-skills/`).
...
|---|---|
| operant tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
When the skill needs a capability, name the proper operant tool in backticks: `terminal`, `file_read`, `file_write`, `patch`, `file_search`, `web_search`, `web_fetch`, `browser` (commands: navigate/snapshot/click/type/scroll/accessibility_tree), `computer_use`, `delegate_task`, `cron` (via `operant cron`). Do NOT name shell utilities the agent already has wrapped (`grep` → `file_search`, `cat` → `file_read`, `sed`/`awk` → `patch`, `find`/`ls` → `file_search`). A CLI-wrapper skill should frame invocations as `terminal(command="<tool> ...", timeout=...)` — bare shell prose ("run `foo --version`") is a review-blocking non-conformance. If the skill depends on an MCP server, name it and document setup in Prerequisites.
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.operant/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Operant (adapted from hermes-agent)` on a contributed skill.** Credit the human first.
