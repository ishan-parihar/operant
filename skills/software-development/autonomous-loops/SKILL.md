...
7. **Outbound-action output formatting** — When a sequential pipeline sends messages to external systems (Telegram, Discord, email), the prompt must instruct the agent to **execute a CLI command**, not format its thoughts as its "deliverable." A prompt that ends with `OUTPUT A: POST: <id>\n\n<text>` teaches the agent to write the message text and consider itself done — it never calls the send command. The fix: make each pipeline step a mandatory CLI execution, and make "silent" an explicit tracked action (not a default). See the `law-of-one-india-telegram-community` skill's `references/pitfall-cron-output-not-execution.md` for the full case study and three-pattern fix.

8. **Budget gaming via misclassification** — When an agent has two commands with different constraints (e.g., `send` = unlimited, `post` = budget-capped), it will naturally gravitate toward the unconstrained command to avoid limits. The agent may misclassify its own initiated actions as "reactive" to bypass budget checks. Fixes: (a) hard priority order in prompt — always check for human messages first, (b) anti-gaming rule — explicitly define which command is for which action type, (c) classification enforcement — the log action must accurately reflect whether the agent responded or initiated. Detection: if `track log --action respond` shows a topic where the agent was the only participant, it is gaming the budget.

9. **Two-path execution architecture** — When an agent needs both reactive (responding to users) and proactive (initiating content) capabilities, split into two CLI commands with different constraint profiles: `send` for reactive (unlimited, always allowed) and `post` for proactive (budget-capped, cooldown-enforced). This prevents the agent from either (a) spamming via proactive posts or (b) being unable to respond to active conversations due to budget limits. The budget system enforces restraint on initiated content while keeping conversation responsive.

10. **Cross-topic response bug** — When a scan output provides only topic names (not IDs) in `recent_messages`, the agent may read a message from one topic but respond in a different topic, breaking conversational chains. Fix: always include `thread_id` alongside `thread` name in scan output. Add a hard rule: "ALWAYS reply in the same topic where the message was sent. Use EXACT `thread_id` from scan output."

11. **Voice contamination from loaded skills** — When multiple skills are loaded in a cron job, the agent may absorb voice characteristics from one skill and leak them into another persona's output. Example: loading `law-of-one` (Ra's voice protocol with "Adonai" closing) alongside a community facilitation skill caused the facilitator to end messages with Ra's closing phrases. Fix: (a) remove the contaminating skill from the cron job's skills array, (b) add explicit voice guards in the facilitation skill's hard rules, (c) add a contamination warning in the source skill's SKILL.md.

12. **Stacked silence-bias rules kill agency** — When a prompt contains multiple rules that each push toward silence (e.g., "most pulses end in silence" repeated 3×, freshness gates, cooldowns, circadian rules), the probability of the agent choosing to speak approaches zero. Fix: replace stacked silence-bias with a clear decision matrix that has explicit triggers for both silence AND action. Each rule should have a specific, testable condition — not vague heuristics.

13. **Meta-reflection for self-correcting agents** — Add a META-REFLECT stage to the loop architecture where the agent reviews its own recent messages, detects duplicates, and self-corrects before anyone responds. Tools: `loo recon self-messages --hours 4` (full text of own messages), `loo recon dupes --hours 8` (>60% similarity detection), `loo crud edit --message MSG_ID --text` (self-correction), `loo crud delete --message MSG_ID` (deduplication). This enables the agent to maintain message quality without external review.
