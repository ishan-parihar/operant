...
This is the standalone spike skill. For the full GSD (Get Shit Done) workflow — persistent `.planning/spikes/` state, MANIFEST tracking across sessions, Given/When/Then verdict format — install the `gsd-build/get-shit-done` CLI separately and follow its own spike workflow.
...
Use operant tools for the research step:
...
Adapted from the GSD (Get Shit Done) project's `/gsd-spike` workflow — MIT © 2025 Lex Christopherson ([gsd-build/get-shit-done](https://github.com/gsd-build/get-shit-done)). The full GSD system offers persistent spike state and MANIFEST tracking via its own CLI; this skill is the standalone, dependency-free version.
