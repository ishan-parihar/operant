...
| **plan** | Phase 0 (Setup): creating structured plans before execution. Writes to `.operant/plans/` | `skill_view("plan")` |
...
| **`code_execution`** | Run Python for citation verification, statistical analysis, data aggregation. Has tool access via RPC. |
...
| **cron `deliver:`** | Notify the user when experiments complete or drafts are ready even if they're not in chat — schedule the check as an operant cron job (`operant cron create`) with a messaging deliver target via the gateway. |
...
→ terminal("ls results/")
→ code_execution("analyze results JSON, compute metrics")
