...
- smoke-test a server with the FastMCP CLI before wiring it into operant or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to operant. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.operant/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.operant/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is operant integration, either:

- configure the server in `~/.operant/operant.toml` (see the `native-mcp` reference), or
...
### operant cannot see the deployed server

The server-building part may be correct while the operant config is not. Load the `native-mcp` reference and configure the server in `~/.operant/operant.toml`, then restart operant.
