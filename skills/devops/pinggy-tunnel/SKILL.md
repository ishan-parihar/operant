...
version: 0.1.0
author: Teknium (teknium1), Operant (adapted from hermes-agent)
license: MIT
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. operant' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
