...
```
browser(command="navigate", url="file:///absolute/path/to/sketches/001-calm-editorial/index.html")
browser(command="snapshot")  # then accessibility_tree — does this layout look clean and readable? Any visible bugs (overlapping text, unstyled elements, broken images)?")
```

the `browser` tool snapshot returns the page text — catches layout bugs that pure source inspection misses (e.g. a font import that silently failed, a flex container that collapsed). Fix and re-navigate until each variant looks right.
...
browser(command="navigate", url="file://$(pwd)/sketches/001-calm-editorial/index.html")
browser(command="snapshot")  # How does this look? Any obvious layout issues?
```
