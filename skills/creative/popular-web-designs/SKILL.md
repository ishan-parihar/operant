...
version: 1.0.0
author: Operant (adapted from hermes-agent + Teknium) (design systems sourced from VoltAgent/awesome-design-md)
license: MIT
...
- Reminders to use `write_file` for HTML creation and the `browser` tool (`accessibility_tree` / `snapshot`) for verification
...
and verify the result with the `browser` tool (`snapshot`) to confirm visual accuracy.
