...
//!
//! Provides `operant checkpoints status`, `operant checkpoints list`,
//! `operant checkpoints prune`, and `operant checkpoints clear`.
...
async fn cmd_status(config: &AppConfig) -> Result<()> {
    let storage = operant_home().join("checkpoints");
