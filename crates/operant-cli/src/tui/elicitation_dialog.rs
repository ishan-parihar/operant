...
//
// Mirrors upstream elicitation_dialog.rs with Operant styling.
// Handles JSON Schema → form field mapping, validation, and user input.
    ...
    pub fn insert_char(&mut self, ch: char) {
        ...
        match &mut field.kind {
            ...
            ElicitationFieldKind::Enum { options } => {
                // Typeahead: find first option whose label starts with the current value + new char
    ...
    pub fn toggle_active(&mut self) {
        ...
        match &mut field.kind {
            ...
            ElicitationFieldKind::MultiEnum { checked, .. } => {
                // Find the focused option (we track sub-cursor via the value field)
