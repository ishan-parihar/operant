...
// Shown briefly in the message area when Operant updates a memory file
// (e.g. ~/.operant/AGENTS.md or a project-local AGENTS.md).
//
...
use crate::tui::overlays::{OPERANT_ACCENT, OPERANT_MUTED, OPERANT_PANEL_BG, OPERANT_TEXT};
...
pub fn render_memory_update_notification(
    ...
    let line = Line::from(vec![
        ...
        Span::styled("\u{1f9e0} ", Style::default().fg(OPERANT_ACCENT)),
        Span::styled("Memory updated in ", Style::default().fg(OPERANT_TEXT)),
        Span::styled(
            display_path,
            Style::default()
                .fg(OPERANT_ACCENT)
        ...
        Span::styled(" \u{00b7} ", Style::default().fg(OPERANT_MUTED)),
        Span::styled(
            "/memory",
            Style::default()
                .fg(OPERANT_ACCENT)
        ...
        Span::styled(" to edit", Style::default().fg(OPERANT_MUTED)),
        Span::styled("  Esc dismiss", Style::default().fg(OPERANT_MUTED)),
    ]);
...
mod tests {
    ...
    fn memory_notif_show_and_dismiss() {
        ...
        state.show("/home/user/.operant/AGENTS.md");
        assert!(state.visible);
        assert_eq!(state.memory_path, "/home/user/.operant/AGENTS.md");
        state.dismiss();
        assert!(!state.visible);
    }
