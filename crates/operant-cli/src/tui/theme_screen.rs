...
pub fn render_theme_screen(frame: &mut Frame, screen: &ThemeScreen, area: Rect) {
    ...
    for (i, theme) in screen.themes.iter().enumerate() {
        ...
        let bg = if is_selected { OPERANT_ACCENT } else { OPERANT_PANEL_BG };
        let fg = if is_selected { Color::White } else { OPERANT_TEXT };
        let desc_fg = if is_selected { Color::Rgb(248, 220, 236) } else { OPERANT_MUTED };
