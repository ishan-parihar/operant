...
use crate::tui::overlays::{centered_rect, OPERANT_PANEL_BG};
...
pub fn render_ask_user_dialog(state: &AskUserDialogState, area: Rect, buf: &mut Buffer) {
    ...
    let title_style = Style::default().fg(TITLE_FG).bg(OPERANT_PANEL_BG).add_modifier(Modifier::BOLD);
    ...
    for wrap_line in word_wrap(&state.question, inner_w) {
        write_line!(
            row,
            Line::from(Span::styled(wrap_line, Style::default().fg(QUESTION_FG).bg(OPERANT_PANEL_BG)))
        );
        row += 1;
        if row >= inner.y + inner.height {
            return;
    ...
    if let Some(ref opts) = state.options {
        for (i, opt) in opts.iter().enumerate() {
            ...
            let style_bg = if is_sel { SELECTED_BG } else { OPERANT_PANEL_BG };
    ...
    if row < inner.y + inner.height {
        ...
        write_line!(row, Line::from(Span::styled(hint, Style::default().fg(HINT_FG).bg(OPERANT_PANEL_BG))));
    }
