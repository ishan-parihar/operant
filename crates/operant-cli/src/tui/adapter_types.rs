...
pub mod voice {
    ...
    impl VoiceRecorder {
        ...
        pub async fn start_recording(&mut self, _tx: tokio::sync::mpsc::Sender<VoiceEvent>) -> std::result::Result<(), String> { Ok(()) }
        pub async fn stop_recording(&mut self) -> std::result::Result<Vec<u8>, String> { Ok(vec![]) }
    }

    pub fn global_voice_recorder() -> std::sync::Arc<std::sync::Mutex<VoiceRecorder>> {
        use std::sync::OnceLock;
        static INSTANCE: OnceLock<std::sync::Arc<std::sync::Mutex<VoiceRecorder>>> = OnceLock::new();
        INSTANCE.get_or_init(|| std::sync::Arc::new(std::sync::Mutex::new(VoiceRecorder::new()))).clone()
    }
}
...
impl TuiApp {
    pub async fn enter(
        ...
        let app = crate::tui::app::App::new(config, cost_tracker, command_registry);
