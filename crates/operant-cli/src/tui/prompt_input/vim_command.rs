//! Vim command-mode (`:` / `/` / visual) handling for the prompt input.
//!
//! Extracted from the original monolith to keep `mod.rs` focused on the
//! core editing state machine. All state lives on `PromptInputState`;
//! this file only adds the command-mode impl block.
