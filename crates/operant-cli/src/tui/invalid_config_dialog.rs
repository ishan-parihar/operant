// invalid_config_dialog.rs — Startup dialog for malformed operant.toml or AGENTS.md.
//
...
impl InvalidConfigDialogState {
    ...
    /// Show the dialog with a settings.toml error.
    ...
    pub fn show_agents_md_error(error: &str) -> Self {
        Self {
            visible: true,
            kind: InvalidConfigKind::AgentsMd,
            error_message: error.to_string(),
            scroll: 0,
        }
    }
...
pub fn render_invalid_config_dialog(
    ...
    let title = match state.kind {
        ...
        InvalidConfigKind::AgentsMd => " Invalid AGENTS.md ",
    ...
    let subtitle = match state.kind {
        InvalidConfigKind::Settings => "~/.operant/operant.toml could not be parsed.",
        InvalidConfigKind::AgentsMd => "AGENTS.md could not be parsed.",
    ...
    let instructions = match state.kind {
        InvalidConfigKind::Settings => vec![
            "  1. Open ~/.operant/operant.toml in a text editor.",
            "  2. Fix the TOML syntax error shown above.",
            "  3. Restart Operant.",
        ],
        InvalidConfigKind::AgentsMd => vec![
