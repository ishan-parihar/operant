...
pub fn render_mcp_view(state: &McpViewState, area: Rect, buf: &mut Buffer) {
    ...
    let title = Line::from(vec![
        Span::styled(" MCP", Style::default().fg(OPERANT_ACCENT).add_modifier(Modifier::BOLD)),
        Span::styled(
    ...
    let footer = Line::from(vec![
        Span::styled(" Tab ", Style::default().fg(OPERANT_ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("switch pane  "),
        Span::styled(" / ", Style::default().fg(OPERANT_ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("filter tools  "),
        Span::styled(" e ", Style::default().fg(OPERANT_ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("error detail  "),
        Span::styled(" a ", Style::default().fg(OPERANT_ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("auth  "),
        Span::styled(" r ", Style::default().fg(OPERANT_ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("reconnect"),
    ]);
...
fn render_server_list(state: &McpViewState, area: Rect, buf: &mut Buffer) {
    ...
    Block::default()
        .title(Span::styled(
            " Servers ",
            Style::default()
                .fg(if focused { OPERANT_ACCENT } else { OPERANT_MUTED })
        ...
        *row += 1;
        for (idx, server) in group {
            ...
            let line = Line::from(vec![Span::styled(
                row_text,
                if sel {
                    Style::default().fg(Color::Black).bg(OPERANT_ACCENT).add_modifier(Modifier::BOLD)
                } else {
...
fn render_tool_list(state: &McpViewState, area: Rect, buf: &mut Buffer) {
    ...
    let search_line = Line::from(vec![
        ...
        Span::styled(
            ...
            Style::default().fg(if state.tool_search.is_empty() { OPERANT_MUTED } else { OPERANT_TEXT }),
        ),
    ]);
