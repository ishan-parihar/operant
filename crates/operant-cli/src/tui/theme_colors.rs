...
mod tests {
    ...
    fn all_themes_return_palette() {
        for theme in ["default", "dark", "light", "solarized", "nord", "dracula", "monokai", "deuteranopia", "unknown"] {
            let palette = ColorPalette::for_theme(theme);
            // Just verify it doesn't panic and returns valid colors
            assert!(matches!(palette.error, Color::Rgb(_, _, _) | Color::Red | Color::White | Color::Black));
        }
    }
    ...
    fn get_message_indicator_color_returns_valid_colors() {
        for role in ["user", "assistant", "system", "tool", "unknown"] {
            ...
            assert!(matches!(color, Color::Rgb(_, _, _) | Color::Cyan | Color::White | Color::Black | Color::DarkGray));
        }
    }
