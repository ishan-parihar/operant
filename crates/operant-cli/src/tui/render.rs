...
fn spinner_color(app: &App) -> Color {
    if let Some(start) = app.streaming.stall_start {
...
pub fn render_app(frame: &mut Frame, app: &App) {
    ...
    let prompt_focused = app.permission.permission_request.is_none() && !app.overlays.history_search_overlay.visible;
    ...
    let suggestions_visible =
        app.permission.permission_request.is_none() && !app.overlays.history_search_overlay.visible;
    ...
    if let Some(ref pr) = app.permission.permission_request {
        render_permission_dialog(frame, pr, size);
    }

    // Rewind flow (takes over screen)
    if app.overlays.rewind_flow.visible {
        render_rewind_flow(frame, &app.overlays.rewind_flow, size);
    }

    // Tasks overlay (Ctrl+T)
    if app.overlays.tasks_overlay.visible {
        render_tasks_overlay(frame, &app.overlays.tasks_overlay, size);
    }

    // New help overlay
    if app.overlays.help_overlay.visible {
        render_help_overlay(frame, &app.overlays.help_overlay, size);
    } else if app.overlays.show_help {
        // Legacy fallback â€” render the simple help overlay
        render_simple_help_overlay(frame, size);
    }

    // History search overlay
    if app.overlays.history_search_overlay.visible {
        render_history_search_overlay(
            frame,
            &app.overlays.history_search_overlay,
    ...
    if app.overlays.settings_screen.visible {
        render_settings_screen(frame, &app.overlays.settings_screen, size);
    }

    // Theme picker overlay
    if app.overlays.theme_screen.visible {
        render_theme_screen(frame, &app.overlays.theme_screen, size);
    }

    if app.overlays.stats_dialog.visible {
        render_stats_dialog(&app.overlays.stats_dialog, size, frame.buffer_mut());
    }

    if app.overlays.mcp_view.visible {
        render_mcp_view(&app.overlays.mcp_view, size, frame.buffer_mut());
    }

    if app.overlays.agents_menu.visible {
        render_agents_menu(&app.overlays.agents_menu, size, frame.buffer_mut());
    }

    if app.overlays.diff_viewer.visible {
        let mut state = app.overlays.diff_viewer.clone();
        render_diff_dialog(&mut state, size, frame.buffer_mut());
    }

    if app.overlays.global_search.visible {
        render_global_search(&app.overlays.global_search, size, frame.buffer_mut());
    }

    if app.overlays.feedback_survey.visible {
        render_feedback_survey(&app.overlays.feedback_survey, size, frame.buffer_mut());
    }

    if app.overlays.memory_file_selector.visible {
        render_memory_file_selector(&app.overlays.memory_file_selector, size, frame.buffer_mut());
    }

    if app.overlays.hooks_config_menu.visible {
        render_hooks_config_menu(&app.overlays.hooks_config_menu, size, frame.buffer_mut());
    }

    // Overage credit upsell banner
    if app.overlays.overage_upsell.visible {
        let banner_h = app.overlays.overage_upsell.height();
        if size.height > banner_h + 4 {
            ...
            render_overage_upsell(&app.overlays.overage_upsell, banner_area, frame.buffer_mut());
        }
    }

    // Voice mode availability notice
    if app.overlays.voice_mode_notice.visible {
        let notice_h = app.overlays.voice_mode_notice.height();
        if size.height > notice_h + 4 {
            ...
            render_voice_mode_notice(&app.overlays.voice_mode_notice, notice_area, frame.buffer_mut());
        }
    }

    // Memory update notification banner (bottom of message area)
    if app.overlays.memory_update_notification.visible {
        let notif_h = app.overlays.memory_update_notification.height();
        if size.height > notif_h + 4 {
            ...
            render_memory_update_notification(
                &app.overlays.memory_update_notification,
                notif_area,
                frame.buffer_mut(),
            );
        }
    ...
    if app.overlays.desktop_upsell.visible {
        render_desktop_upsell_startup(&app.overlays.desktop_upsell, size, frame.buffer_mut());
    }

    // Import-config preview dialog
    if app.overlays.import_config_dialog.visible {
        render_import_config_dialog(frame, &app.overlays.import_config_dialog, size);
    }

    // Invalid config/settings dialog (shown when settings.json or AGENTS.md is malformed)
    if app.overlays.invalid_config_dialog.visible {
        render_invalid_config_dialog(frame, &app.overlays.invalid_config_dialog, size);
    }

    // Bypass-permissions confirmation dialog (topmost — rendered last so it sits above all)
    if app.overlays.bypass_permissions_dialog.visible {
        render_bypass_permissions_dialog(frame, &app.overlays.bypass_permissions_dialog, size);
    }

    // File injection warning dialog (shown when oversized/binary files detected)
    if app.overlays.file_injection_dialog.visible {
        render_file_injection_dialog(frame, &app.overlays.file_injection_dialog, size);
    }
    ...
    if app.overlays.ask_user_dialog.visible {
        render_ask_user_dialog(&app.overlays.ask_user_dialog, size, frame.buffer_mut());
    }

    // First-launch onboarding dialog (shown after bypass dialog, below elicitation)
    if app.overlays.onboarding_dialog.visible {
        render_onboarding_dialog(frame, &app.overlays.onboarding_dialog, size);
    }

    // /effort picker
    if app.overlays.effort_picker.visible {
        crate::effort_picker::render_effort_picker(frame, &app.overlays.effort_picker, size);
    }

    // Import-config source picker
    if app.overlays.import_config_picker.visible {
        render_dialog_select(frame, &app.overlays.import_config_picker, size);
    }

    // Connect-a-provider dialog (/connect command)
    if app.overlays.connect_dialog.visible {
        render_dialog_select(frame, &app.overlays.connect_dialog, size);
    }

    // API key input dialog (opened from /connect for key-based providers)
    if app.overlays.key_input_dialog.visible {
        render_key_input_dialog(frame, &app.overlays.key_input_dialog, size);
    }

    // Custom provider URL + API key dialog.
    if app.overlays.custom_provider_dialog.visible {
        render_custom_provider_dialog(frame, &app.overlays.custom_provider_dialog, size);
    }

    // "Free" composite-provider setup dialog (Zen + OpenRouter).
    if app.overlays.free_mode_dialog.visible {
        crate::free_mode_dialog::render_free_mode_dialog(frame, &app.overlays.free_mode_dialog, size);
    }

    // Device code / browser auth dialog (GitHub Copilot, Anthropic OAuth)
    if app.overlays.device_auth_dialog.visible {
        render_device_auth_dialog(frame, &app.overlays.device_auth_dialog, size);
    }

    // Ctrl+K command palette
    if app.overlays.command_palette.visible {
        render_dialog_select(frame, &app.overlays.command_palette, size);
    }

    // MCP elicitation dialog (highest priority modal — rendered last to sit on top)
    if app.overlays.elicitation.visible {
        render_elicitation_dialog(&app.overlays.elicitation, size, frame.buffer_mut());
    }

    // Model picker overlay
    if app.overlays.model_picker.visible {
        render_model_picker(&app.overlays.model_picker, size, frame.buffer_mut());
    }

    // Session browser overlay
    if app.overlays.session_browser.visible {
        render_session_browser(&app.overlays.session_browser, size, frame.buffer_mut());
    }

    // Session branching overlay
    if app.overlays.session_branching.visible {
        render_session_branching(&app.overlays.session_branching, size, frame.buffer_mut());
    }

    // Debug overlay (bottom-right corner, toggleable with Ctrl+Shift+D or /debug)
    if app.show_debug_overlay {
        if let Some(ref debug_state) = app.tui_debug_state {
            let lines = debug_state.render_debug_overlay(size.width);
            let overlay_height = lines.len() as u16 + 2;
            let overlay_width = 42.min(size.width);
            let overlay_area = Rect {
                x: size.x + size.width.saturating_sub(overlay_width + 1),
                y: size.y + size.height.saturating_sub(overlay_height + 1),
                width: overlay_width,
    ...
    if app.overlays.export_dialog.visible {
        render_export_dialog(frame, &app.overlays.export_dialog, size);
    }

    // Context visualization overlay
    if app.overlays.context_viz.visible {
        render_context_viz(
            frame,
            &app.overlays.context_viz,
            size,
    ...
    // MCP approval dialog
    if app.overlays.mcp_approval.visible {
        render_mcp_approval_dialog(&app.overlays.mcp_approval, size, frame.buffer_mut());
    }

    // Always show error modals on top of everything (highest priority)
    if let Some(notif) = app.overlays.notifications.current() {
        if notif.kind == NotificationKind::Error {
            let is_welcome_screen = app.messages.is_empty()
                && app.streaming.streaming_text.is_empty()
                && app.streaming.streaming_thinking.is_empty()
                && app.tool_use_blocks.is_empty();
            render_error_modal(
                frame,
                size,
                notif,
                app.overlays.error_modal_scroll_offset,
    ...
    if !modal_active && app.overlays.notifications.current().is_some() {
        render_notification_banner(frame, &app.overlays.notifications, size);
    }
...
fn render_context_menu(frame: &mut Frame, app: &App) {
    if let Some(menu) = app.overlays.context_menu_state {
...
fn render_messages(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let lines = if app.overlays.global_search.visible && !app.overlays.global_search.query.is_empty() {
        let query_lc = app.overlays.global_search.query.to_lowercase();
        lines
            .into_iter()
            .map(|mut item| {
...
fn render_message_items(app: &App, width: u16) -> Vec<RenderedLineItem> {
    let streaming =
        app.streaming.is_streaming || !app.streaming.streaming_text.is_empty() || !app.streaming.streaming_thinking.is_empty();
    ...
    let full_key = MessageLinesCacheKey {
        width,
        transcript_version: app.streaming.transcript_version.get(),
...
fn render_input(frame: &mut Frame, app: &App, area: Rect, focused: bool) {
    ...
    render_prompt_input(
        ...
        app.accent_color,
        app.overlays.settings_screen.cursor_blink_enabled,
    );
}
...
fn should_render_status_row(app: &App) -> bool {
    ...
    app.voice_recording
        || app.streaming.last_turn_elapsed.is_some()
        || (!app.streaming.is_streaming && app.status_message.is_some())
        || (app.streaming.is_streaming && interesting_stream_status)
}
...
fn render_status_row(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let spans = if app.voice_recording {
    ...
    } else if app.streaming.is_streaming {
        ...
        let raw_label = app
            ...
            .or_else(|| app.streaming.spinner_verb.as_deref())
            .unwrap_or("Thinking");
        ...
        (app.streaming.last_turn_verb, app.streaming.last_turn_elapsed.as_deref())
    {
...
fn render_footer(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let left_spans: Vec<Span> = if app.voice_recording {
        ...
        if spans.is_empty() && app.streaming.is_streaming {
            spans.push(Span::styled(
                "esc interrupt",
                Style::default().fg(Color::DarkGray),
            ));
    ...
    let right_spans: Vec<Span> = {
        ...
        if app.overlays.settings_screen.show_git_branch {
        ...
        if app.overlays.settings_screen.show_cwd {
