// file_injection.rs — Parse @file references in user input with size limits and binary detection.
//
// Mirrors upstream file_injection.rs with Operant-specific config integration.
...
impl AtFileRef {
    /// Returns true if this reference was successfully resolved with contents.
...
mod tests {
    ...
    #[test]
    fn test_parse_at_refs_tilde_expansion() {
        let temp = TempDir::new().unwrap();
        let file_path = temp.path().join("test.txt");
        fs::write(&file_path, "home content").unwrap();

        // We can't easily test ~ expansion without knowing HOME, but we can
        // verify the code path doesn't panic
        let input = format!("Check @{}", file_path.display());
        let (within, _oversized) = parse_at_refs(&input, temp.path(), 1024);
        assert_eq!(within.len(), 1);
    }
}
