...
use operant_core::config::{AppConfig, McpServerConfig, McpTransportKind};
...
pub struct TuiApp {
    ...
    agent: Option<Arc<OperantAgent>>,
    event_tx: mpsc::Sender<AgentEvent>,
    event_rx: mpsc::Receiver<AgentEvent>,
    run_handle: Option<JoinHandle<operant_core::Result<Message>>>,
...
mod tests {
    ...
    async fn join_errors_stay_inside_tui_state() {
        ...
        let run_handle: tokio::task::JoinHandle<operant_core::Result<Message>> =
            task::spawn(async move {
                panic!("boom");
                #[allow(unreachable_code)]
                Ok(operant_core::client::Message::assistant("never"))
            });
