//! Operant MCP Server — expose Operant tools as an MCP server over stdio.
//!
//! Implements the Model Context Protocol (MCP) server side, letting any MCP
//! client (Claude Desktop, VS Code, etc.) discover and call Operant tools.
//!
//! ## Protocol
...
//!
//! ## Tool Discovery
//! All tools registered in the Operant ToolRegistry (built-in + plugin + MCP
...
async fn build_tool_registry(config: &AppConfig) -> Result<ToolRegistry> {
    let client = operant_core::client::OpenAIClient::new(operant_core::client::ClientConfig::from(
        &config.client,
    ));
...
async fn connect_mcp_server(
    ...
    server: &operant_core::config::McpServerConfig,
) -> Result<()> {
...
/// Convert a Operant ToolSchema to an MCP tool definition object.
...
fn handle_resources_list(request: &JsonRpcRequest) -> JsonRpcResponse {
    let resources = vec![
        serde_json::json!({
            ...
            "description": "List of all tools registered in the Operant tool registry",
            "mimeType": "application/json",
        }),
        serde_json::json!({
            "uri": "operant://status",
            "name": "Server Status",
            "description": "Operant MCP server status information",
            "mimeType": "application/json",
        }),
        serde_json::json!({
            "uri": "operant://config",
            "name": "Server Configuration",
            "description": "Active Operant server configuration (non-sensitive fields only)",
            "mimeType": "application/json",
        }),
    ];
...
fn handle_resources_read(request: &JsonRpcRequest) -> JsonRpcResponse {
    ...
    let content = match uri {
        "operant://status" => serde_json::json!({
            "contents": [{
                "uri": uri,
                "mimeType": "application/json",
        ...
        "operant://tools" | "operant://config" => serde_json::json!({
            "contents": [{
                "uri": uri,
                "mimeType": "application/json",
                "text": serde_json::to_string_pretty(&serde_json::json!({
...
mod tests {
    use super::*;
    use operant_core::tools::{OperantTool, ToolResult};
    ...
    impl OperantTool for TestEchoTool {
        fn name(&self) -> &str {
            "echo"
        }
    ...
    fn test_resources_list() {
        ...
        assert!(resources.iter().any(|r| r["uri"] == "operant://tools"));
        assert!(resources.iter().any(|r| r["uri"] == "operant://status"));
    }
