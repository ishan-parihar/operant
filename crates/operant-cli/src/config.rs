...
pub const MIGRATION_STEPS: &[(&str, &str, &str)] = &[
    ...
    ("0.4.0", "1.0.0", "Skills moved under operant.skills_dir"),
];
...
pub fn known_env_vars() -> Vec<EnvVarInfo> {
    vec![
        ...
        EnvVarInfo {
            name: "HERMES_HOME",
            description: "Operant data directory override",
            default: Some("~/.operant"),
...
fn apply_migration_step(config: &mut Value, from: &str, to: &str) -> ConfigResult<()> {
    match (from, to) {
        ...
        ("0.4.0", "1.0.0") => {
            // Move skills config under operant.skills_dir
            if let Some(map) = config.as_mapping_mut() {
                if let Some(skills_val) = map.remove(&Value::String("skills".to_string())) {
                    if let Some(skills_map) = skills_val.as_mapping() {
                        ...
                        {
                            let operant_key = Value::String("operant".to_string());
                            let operant_entry = map
                                .entry(operant_key)
                                .or_insert_with(|| Value::Mapping(serde_yaml::Mapping::new()));
                            if let Some(operant_map) = operant_entry.as_mapping_mut() {
                                operant_map.insert(
