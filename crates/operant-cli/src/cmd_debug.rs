...
//!
//! Provides `operant debug <subcommand>`.
...
async fn cmd_share(config: &AppConfig, local_only: bool, no_upload: bool) -> Result<()> {
    ...
    println!("  operant debug share --local | curl -F 'f=@-' https://paste.rs/");
    println!();
    println!("Or save to a file:");
    println!("  operant debug share --local > operant_debug_report.txt");
...
async fn cmd_delete(_config: &AppConfig, id: &str) -> Result<()> {
    ...
    println!("The placeholder file in ~/.operant/debug/ has been removed.");
    Ok(())
}
...
fn gather_report(config: &AppConfig) -> Result<String> {
    ...
    sections.insert("Operant Config".to_string(), cfg);
    ...
    output.push_str("Operant Debug Report\n");
    output.push_str("═══════════════════\n");
