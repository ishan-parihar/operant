...
use operant_core::platform::detect_shell;
...
fn build_autonomous_query(
    ...
    format!(
        "You are running in operant-rs autonomous coding mode.\n\
...
mod tests {
    ...
    fn sample_config() -> AppConfig {
        ...
        config.autonomous.commit_message = "Auto-commit by operant-rs".to_string();
    ...
    fn init_disposable_repo(todo_contents: &str) -> PathBuf {
        ...
        git_success(&repo_root, &["config", "user.name", "Operant Test"]);
        git_success(&repo_root, &["config", "user.email", "operant@example.com"]);
