//! RL Training CLI — reinforcement learning commands for Operant.
//!
//! Ported from operant-agent/rl_cli.py concepts.
...
async fn cmd_evaluate(config: &AppConfig, state: Option<String>) -> Result<()> {
    ...
    if state_count == 0 {
        println!("No Q-table data available. Run `operant rl train` first.");
        return Ok(());
    }

    let rl_state = trainer.get_state();

    println!("Operant RL Evaluation");
    println!("{}", "-".repeat(50));
...
async fn cmd_list_environments() -> Result<()> {
    ...
    println!();
    println!("Use `operant rl doctor` to check your setup.");
    Ok(())
}

/// Check RL environment setup.
async fn cmd_doctor(config: &AppConfig) -> Result<()> {
    println!("Operant RL Doctor Check");
    println!("{}", "-".repeat(50));
    ...
    if let Some(home) = dirs::home_dir() {
        let operant_home = home.join(".operant");
        if operant_home.exists() {
            let (found, msg) = check_tinker_atropos(&operant_home);
            if found {
    ...
    match trainer.load() {
        Ok(()) => {
            ...
            if states > 0 {
                ...
                println!("ℹ️  No saved training data. Run `operant rl train` to start.");
            }
        }
        Err(e) => {
...
async fn cmd_run(
    ...
    println!("🎯 Operant RL Training Agent");
    println!("{}", "=".repeat(60));
    ...
    if !missing.is_empty() {
        ...
        println!("Set them in ~/.operant/.env or your shell.");
        return Ok(());
    }
    ...
    // Note: Full agent integration requires wiring into OperantAgent.
    ...
    println!(
        "Task: {}\n\
         ...
         \n\
         (Full agent loop integration requires python operant-agent for now.)",
        prompt
    );
