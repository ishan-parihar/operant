...
mod tests {
    ...
    fn test_add_mount() {
        ...
        cf.add_mount("github", "/root/.hermes/github_token.json");
        assert!(!cf.is_empty());
        let mounts = cf.get_mounts("github");
        assert!(mounts.is_some());
        assert_eq!(mounts.unwrap().len(), 1);
        assert_eq!(mounts.unwrap()[0], "/root/.hermes/github_token.json");
    }
    ...
    fn test_add_cache() {
        let mut cf = CredentialFiles::new();
        cf.add_cache("documents", "/root/.hermes/cache/documents");
        assert!(!cf.is_empty());
        let caches = cf.get_cache("documents");
        assert!(caches.is_some());
        assert_eq!(caches.unwrap().len(), 1);
        assert_eq!(caches.unwrap()[0], "/root/.hermes/cache/documents");
    }
    ...
    fn test_multiple_mounts_same_key() {
        let mut cf = CredentialFiles::new();
        cf.add_mount("docker", "/root/.hermes/config.json");
        cf.add_mount("docker", "/root/.hermes/auth.json");
        let mounts = cf.get_mounts("docker").unwrap();
        assert_eq!(mounts.len(), 2);
        assert_eq!(mounts[0], "/root/.hermes/config.json");
        assert_eq!(mounts[1], "/root/.hermes/auth.json");
    }

    #[test]
    fn test_multiple_cache_same_key() {
        let mut cf = CredentialFiles::new();
        cf.add_cache("images", "/root/.hermes/cache/images");
        cf.add_cache("images", "/root/.hermes/cache/screenshots");
    ...
    fn test_clear() {
        let mut cf = CredentialFiles::new();
        cf.add_mount("github", "/root/.hermes/token.json");
        cf.add_cache("docs", "/root/.hermes/cache/documents");
        assert!(!cf.is_empty());
        cf.clear();
        assert!(cf.is_empty());
