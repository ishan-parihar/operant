...
//!
//! Mirrors the Python `SessionStore` from `hermes-agent/gateway/session.py`:
...
impl PersistentSessionStore {
    ...
    /// Find a session by its Hermes session ID (legacy API).
    pub fn get_hermes_session(&self, hermes_session_id: &str) -> Option<PlatformSession> {
        self.entries
            .read()
            .unwrap()
            .values()
            .find(|e| e.session_id == hermes_session_id)
            .map(|e| PlatformSession {
                session_id: e.session_id.clone(),
                platform: e.platform.clone().unwrap_or_default(),
                platform_user_id: e
