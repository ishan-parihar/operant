...
pub async fn send_cdp_command(url: &str, command: &Value) -> Result<Value> {
    let (ws_stream, _): (WsStream, _) = connect_async(url).await.map_err(|e| {
        Error::ToolExecution {
            name: "cdp".into(),
    ...
    let cmd_str = serde_json::to_string(command).map_err(|e| {
        Error::ToolExecution {
            name: "cdp".into(),
    ...
    write.send(Message::Text(cmd_str.into())).await.map_err(|e| {
        Error::ToolExecution {
            name: "cdp".into(),
            source: e.into(),
        }
        ...
        if let Message::Text(text) = msg {
            ...
            if let Some(error) = response.get("error") {
                return Err(Error::ToolExecution {
                    name: "cdp".into(),
                    source: std::io::Error::new(std::io::ErrorKind::Other, error.to_string()).into(),
                });
            }
    ...
    Err(Error::ToolExecution {
        name: "cdp".into(),
        source: std::io::Error::new(std::io::ErrorKind::ConnectionAborted,
