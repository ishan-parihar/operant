...
impl WebSearchProvider for SearXNGProvider {
    ...
    async fn search(&self, query: &str, num_results: usize) -> Result<Vec<WebSearchResult>> {
        let url = format!("{}/search?format=json&q={}", self.base_url, urlencoding(query));
        let client = reqwest::Client::new();
        let resp = client.get(&url).send().await?;
        let data: serde_json::Value = resp.json().await?;
        let results = data["results"].as_array().ok_or_else(|| Error::ParseResponse("SearXNG: missing results".into()))?;
        Ok(results.iter().take(num_results).map(|r| WebSearchResult {
