...
impl Dispatcher {
    pub fn new(conn: Arc<Mutex<Connection>>) -> Self {
        Self { db: conn, poll_interval: std::time::Duration::from_secs(5) }
    }
    ...
    pub fn complete_run(&self, task_id: &str, run_id: i64, outcome: &str, summary: Option<&str>) -> Result<(), Error> {
        let conn = self.db.lock().unwrap();
        let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() as i64;
        conn.execute(
    ...
    pub fn gc(&self, older_than_days: i64) -> Result<(usize, usize, usize), Error> {
        let conn = self.db.lock().unwrap();
        let cutoff = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() as i64 - older_than_days * 86400;
        ...
        let removed_tasks = conn.execute(
            "DELETE FROM tasks WHERE status = 'archived' AND completed_at < ?1", params![cutoff],
        ).map_err(|e| Error::Agent(format!("GC tasks: {}", e)))?;
        Ok((removed_tasks as usize, removed_runs as usize, removed_events as usize))
    }
}
