...
impl KanbanDiagnostics {
    ...
    fn check_stale_tasks(&self) -> Result<Vec<DiagnosticIssue>, Error> {
        ...
        let issues: Vec<DiagnosticIssue> = stmt.query_map(params![cutoff], |row| {
            ...
            Ok(DiagnosticIssue {
                ...
                task_id: id.clone(),
                title: title.clone(),
                description: format!("Task '{}' has been in 'running' status for {}s without heartbeat", id, ago),
                action: format!("Run `hermes kanban reclaim {}` to reset to 'todo' status", id),
            })
    ...
    fn check_zombie_runs(&self) -> Result<Vec<DiagnosticIssue>, Error> {
        ...
        let issues: Vec<DiagnosticIssue> = stmt.query_map([], |row| {
            ...
            Ok(DiagnosticIssue {
                ...
                title: title.clone(),
                description: format!("Run {} is 'running' but task '{}' status is not 'running'", run_id, task_id),
                action: format!("Run `UPDATE task_runs SET status='orphaned' WHERE id={}`", run_id),
            })
        }).map_err(|e| Error::Agent(format!("Query error: {}", e)))?
        .filter_map(|r| r.ok())
        .collect();
    ...
    fn check_orphan_links(&self) -> Result<Vec<DiagnosticIssue>, Error> {
        ...
        let issues: Vec<DiagnosticIssue> = stmt.query_map([], |row| {
            ...
            Ok(DiagnosticIssue {
                ...
                description: format!("Link between '{}' and '{}' references missing task", parent_id, child_id),
                action: format!("Run `hermes kanban unlink {} {}` to remove", parent_id, child_id),
            })
        }).map_err(|e| Error::Agent(format!("Query error: {}", e)))?
        .filter_map(|r| r.ok())
        .collect();
