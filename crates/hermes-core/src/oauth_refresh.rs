...
//!
//! Ported from hermes-agent's `agent/credential_pool.py` and `hermes_cli/auth.py`.
...
impl OAuthRefresher {
    ...
    async fn refresh_anthropic(
        ...
        let use_json = entry.source.ends_with("hermes_pkce");
        ...
        if !resp.status().is_success() {
            ...
            if let Ok(err) = serde_json::from_str::<serde_json::Value>(&text) {
                ...
                if desc.to_lowercase().contains("reuse") {
                    message = "Nous Portal detected refresh-token reuse. Re-authenticate with `hermes auth add nous`.".to_string();
                    relogin = true;
                }
            }
