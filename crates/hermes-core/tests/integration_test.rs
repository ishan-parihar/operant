//! Integration tests for the hermes-core tool system.
//!
...
async fn test_schema_deserialize_round_trip() {
    ...
    for original in &schemas {
        ...
        let deserialized: hermes_core::schema::ToolSchema =
...
//
// The tool modules in hermes-core are designed as independent units. Each tool
...
fn zero_dep_tools() -> Vec<(&'static str, Box<dyn HermesTool>)> {
    vec![
