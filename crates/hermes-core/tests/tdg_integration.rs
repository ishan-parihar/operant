...
async fn test_tdg_provider_initialization() {
    ...
    let provider = tdg_rust::memory_provider::TdgMemoryProvider::new(storage.clone());
...
async fn test_tdg_provider_prefetch_empty() {
    let storage = test_storage_dir();
    let provider = tdg_rust::memory_provider::TdgMemoryProvider::new(storage);
    provider.initialize("test-session").await.unwrap();
...
async fn test_tdg_provider_handle_connect() {
    ...
    let id1: String = serde_json::from_str(&create1).unwrap()
    ...
    let id2: String = serde_json::from_str(&create2).unwrap()
        .get("id").unwrap().as_str().unwrap().to_string();
    let result = provider.handle_tool_call(
        "tdg_connect",
        serde_json::json!({
...
async fn test_tdg_provider_handle_get_related() {
    ...
    let id: String = serde_json::from_str(&create).unwrap()
        .get("id").unwrap().as_str().unwrap().to_string();
    let result = provider.handle_tool_call(
        "tdg_get_related",
...
fn test_build_memory_provider_tdg() {
    let storage = test_storage_dir();
    let provider = tdg_rust::build_memory_provider("tdg", storage);
    assert_eq!(provider.name(), "tdg");
}
...
fn test_build_memory_provider_default() {
    let storage = test_storage_dir();
    let provider = tdg_rust::build_memory_provider("unknown", storage);
