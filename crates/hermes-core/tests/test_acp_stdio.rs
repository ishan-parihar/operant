...
use hermes_core::acp::{AcpHandler, AgentState};
use std::sync::Arc;
...
async fn test_ping_pong() {
    ...
    let request = hermes_core::acp::parse_request(raw).unwrap();
    let (response, should_shutdown) = hermes_core::acp::dispatch(&request, &*handler).await;
...
async fn test_status() {
    ...
    assert_eq!(result["server"]["name"], "hermes-acp");
}
...
async fn test_invalid_json_parse_error() {
    let result = hermes_core::acp::parse_request("not valid json");
...
async fn test_round_trip_serialization() {
    ...
    let request = hermes_core::acp::parse_request(raw).unwrap();
    let (response, _) = hermes_core::acp::dispatch(&request, &*handler).await;
    let serialized = hermes_core::acp::serialize_response(&response).unwrap();
