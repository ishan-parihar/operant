...
impl PerSenderTracker {
    ...
    fn current_key() -> String {
        operant_api::TOOL_LOOP_THREAD_ID
            .try_with(|v| v.clone())
            .ok()
            .flatten()
...
mod tests {
    ...
    fn absolute_path_inside_workspace_allowed_when_workspace_only() {
        let p = SecurityPolicy {
            workspace_dir: PathBuf::from("/home/user/.operant/workspace"),
            workspace_only: true,
            ..SecurityPolicy::default()
        };
        // Absolute path inside workspace should be allowed
        assert!(p.is_path_allowed("/home/user/.operant/workspace/images/example.png"));
        assert!(p.is_path_allowed("/home/user/.operant/workspace/file.txt"));
    ...
    fn absolute_path_in_allowed_root_permitted_when_workspace_only() {
        let p = SecurityPolicy {
            workspace_dir: PathBuf::from("/home/user/.operant/workspace"),
            workspace_only: true,
            allowed_roots: vec![PathBuf::from("/home/user/.operant/shared")],
            ..SecurityPolicy::default()
        };
        // Path in allowed root should be permitted
        assert!(p.is_path_allowed("/home/user/.operant/shared/data.txt"));
    ...
    fn workspace_only_false_allows_resolved_outside_workspace() {
        let workspace = std::env::temp_dir().join("operant_test_ws_only_false");
    ...
    fn workspace_only_true_blocks_resolved_outside_workspace() {
        let workspace = std::env::temp_dir().join("operant_test_ws_only_true");
        let _ = std::fs::create_dir_all(&workspace);
        let canonical_workspace = workspace
            .canonicalize()
            .unwrap_or_else(|_| workspace.clone());
    ...
    fn resolved_path_blocks_outside_workspace() {
        let workspace = std::env::temp_dir().join("operant_test_resolved_path");
        let _ = std::fs::create_dir_all(&workspace);
        ...
        let canonical_temp = std::env::temp_dir()
            .canonicalize()
            .unwrap_or_else(|_| std::env::temp_dir());
        let outside = canonical_temp.join("outside_workspace_operant");
        assert!(
    ...
    fn resolved_path_blocks_root_escape() {
        let policy = SecurityPolicy {
            workspace_dir: PathBuf::from("/home/operant_user/project"),
            ..SecurityPolicy::default()
        };
    ...
    fn resolved_path_blocks_symlink_escape() {
        ...
        let root = std::env::temp_dir().join("operant_test_symlink_escape");
    ...
    fn allowed_roots_permits_paths_outside_workspace() {
        ...
        let root = std::env::temp_dir().join("operant_test_allowed_roots");
        let workspace = root.join("workspace");
    ...
    fn resolve_tool_path_normalizes_workspace_prefixed_relative_paths() {
        let p = SecurityPolicy {
            workspace_dir: PathBuf::from("/operant-data/workspace"),
            ..SecurityPolicy::default()
        };
        let resolved = p.resolve_tool_path("operant-data/workspace/scripts/daily.py");
        assert_eq!(
            resolved,
            PathBuf::from("/operant-data/workspace/scripts/daily.py")
        );
    }
    ...
    fn runtime_config_paths_are_protected() {
        let workspace = PathBuf::from("/tmp/operant-profile/workspace");
        let policy = SecurityPolicy {
            workspace_dir: workspace.clone(),
            ..SecurityPolicy::default()
        };
