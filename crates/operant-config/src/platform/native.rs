use operant_api::runtime_traits::RuntimeAdapter;
...
impl NativeRuntime {
    /// Create a native runtime (full host access).
    pub fn new() -> Self {
        Self
    }
}
...
mod tests {
    ...
    #[test]
    fn native_storage_path_contains_operant() {
        let path = NativeRuntime::new().storage_path();
        assert!(path.to_string_lossy().contains("operant"));
    }
