...
/// Tokens are cached to `~/.config/operant/copilot/` and refreshed
/// automatically.
...
impl CopilotProvider {
    pub fn new(github_token: Option<&str>) -> Self {
        let token_dir = directories::ProjectDirs::from("", "", "operant")
            ...
            .unwrap_or_else(|| {
                ...
                std::env::temp_dir().join(format!("operant-copilot-{user}"))
            });
