...
impl OpenAiCompatibleProvider {
    ...
    fn http_client(&self) -> Client {
        ...
        if has_user_agent || has_extra_headers {
            ...
            let builder = operant_config::schema::apply_runtime_proxy_to_builder(
                builder,
                "provider.compatible",
            );
        ...
        operant_config::schema::build_runtime_proxy_client_with_timeouts(
            "provider.compatible",
            timeout,
            10,
        )
    ...
    fn streaming_http_client(&self) -> Client {
        ...
        let builder =
            operant_config::schema::apply_runtime_proxy_to_builder(builder, "provider.compatible");
    ...
    fn tool_specs_to_openai_format(
        ...
        tools
            ...
            .map(|tool| {
                let params =
                    operant_api::schema::SchemaCleanr::clean_for_openai(tool.parameters.clone());
                serde_json::json!({
                    "type": "function",
                    "function": {
                        "name": tool.name,
...
struct ToolCall {
    ...
    /// See [`operant_api::ToolCall::extra_content`].
    #[serde(default, skip_serializing_if = "Option::is_none")]
                ...
                Ok(bytes) => {
                    ...
                    while let Some(pos) = buffer.find('\n') {
                        ...
                        if let Some(usage) = chunk.usage.as_ref() {
                            let token_usage = operant_api::provider::TokenUsage {
...
impl OpenAiCompatibleProvider {
    ...
    fn convert_tool_specs(
        tools: Option<&[operant_api::tool::ToolSpec]>,
    ) -> Option<Vec<serde_json::Value>> {
        tools.map(|items| {
            items
                .iter()
                .map(|tool| {
                    let params = operant_api::schema::SchemaCleanr::clean_for_openai(
    ...
    fn with_prompt_guided_tool_instructions(
        ...
        let instructions = operant_api::provider::build_tool_instructions_text(tools);
...
impl Provider for OpenAiCompatibleProvider {
    fn capabilities(&self) -> operant_api::provider::ProviderCapabilities {
        operant_api::provider::ProviderCapabilities {
    ...
    fn stream_chat(
        ...
        let tools_owned: Option<Vec<operant_api::tool::ToolSpec>> =
            request.tools.map(<[operant_api::tool::ToolSpec]>::to_vec);
...
mod tests {
    ...
    fn prompt_guided_tool_fallback_injects_system_instruction() {
        ...
        let tools = vec![operant_api::tool::ToolSpec {
    ...
    fn tool_specs_convert_to_openai_format() {
        let specs = vec![operant_api::tool::ToolSpec {
