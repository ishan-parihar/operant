...
///
/// ```rust,ignore
/// use operant::providers::telnyx::TelnyxProvider;
/// use operant::providers::Provider;
///
...
impl TelnyxProvider {
    ...
    /// 2. `OPERANT_API_KEY` environment variable (fallback)
...
impl Provider for TelnyxProvider {
    ...
    async fn chat_with_system(
        ...
        let api_key = self.api_key.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "Telnyx API key not set. Set TELNYX_API_KEY environment variable or run `operant onboard`."
            )
        })?;
