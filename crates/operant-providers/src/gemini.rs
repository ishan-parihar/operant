...
//! - Operant auth-profiles OAuth tokens
...
impl GeminiProvider {
    ...
    fn http_client(&self) -> Client {
        operant_config::schema::build_runtime_proxy_client_with_timeouts("provider.gemini", 120, 10)
    }
        ...
        let auth = self.auth.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "Gemini API key not found. Options:\n\
                 ...
                 3. Run `operant auth login --provider gemini`\n\
                 4. Get an API key from https://aistudio.google.com/app/apikey\n\
                 5. Run `operant onboard` to configure"
            )
        })?;
        ...
        let (mut oauth_token, mut project) = match auth {
            ...
            GeminiAuth::ManagedOAuth => {
                ...
                let token = auth_service
                    ...
                    .ok_or_else(|| {
                        anyhow::anyhow!(
                            "Gemini auth profile not found. Run `operant auth login --provider gemini`."
                        )
                    })?;
                let proj = self.resolve_oauth_project(&token).await?;
                (Some(token), Some(proj))
            ...
            match auth {
                GeminiAuth::ManagedOAuth => {
                    ...
                    let _token = auth_service
                        ...
                        .ok_or_else(|| {
                            anyhow::anyhow!(
                                "Gemini auth profile not found or expired. Run: operant auth login --provider gemini"
                            )
                        })?;
