...
pub struct ProviderRuntimeOptions {
    ...
    pub operant_dir: Option<PathBuf>,
    ...
    /// These are merged from the config file and `OPERANT_EXTRA_HEADERS` env var.
...
pub fn provider_runtime_options_from_config(
    ...
    ProviderRuntimeOptions {
        ...
        provider_api_url: fallback
            ...
            .map(ToString::to_string),
        operant_dir: config.config_path.parent().map(PathBuf::from),
...
/// 3. Generic fallback variables (`OPERANT_API_KEY`, `API_KEY`)
///
...
fn resolve_provider_credential(name: &str, credential_override: Option<&str>) -> Option<String> {
    ...
    for env_var in ["OPERANT_API_KEY", "API_KEY"] {
        if let Ok(value) = std::env::var(env_var) {
            let value = value.trim();
            if !value.is_empty() {
                return Some(value.to_string());
...
/// case.
pub fn default_provider_config(name: &str) -> operant_config::schema::ModelProviderConfig {
    use operant_config::schema::ModelProviderConfig;
...
fn create_provider_with_url_and_options(
    ...
    match name {
        ...
        "ollama" => {
            let env_url = std::env::var("OPERANT_PROVIDER_URL").ok();
        ...
        "gemini" | "google" | "google-gemini" => {
            let state_dir = options.operant_dir.clone().unwrap_or_else(|| {
                directories::UserDirs::new().map_or_else(
                    || PathBuf::from(".operant"),
                    |dirs| dirs.home_dir().join(".operant"),
                )
            });
        ...
        _ => anyhow::bail!(
            "Unknown provider: {name}. Check README for supported providers or run `operant onboard` to reconfigure.\n\
...
pub struct ProviderInfo {
    ...
    /// `operant integration info <name>`. Empty string suppresses the
    /// description line.
...
/// Return the list of all known providers for display in `operant providers list`.
///
...
mod tests {
    ...
    // `tls_built_in_root_certs` is gated on `rustls-tls-native-roots-no-provider`.
    ...
    fn resolve_qwen_oauth_context_prefers_explicit_override() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/operant-qwen-oauth-home-{}", std::process::id());
    ...
    fn resolve_qwen_oauth_context_uses_env_token_and_resource_url() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/operant-qwen-oauth-home-{}-env", std::process::id());
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, Some("oauth-token"));
    ...
    fn resolve_qwen_oauth_context_reads_cached_credentials_file() {
        let _env_lock = env_lock();
        let fake_home = format!("/tmp/operant-qwen-oauth-home-{}-file", std::process::id());
    ...
    fn resolve_qwen_oauth_context_placeholder_does_not_use_dashscope_fallback() {
        ...
        let fake_home = format!(
            "/tmp/operant-qwen-oauth-home-{}-placeholder",
            std::process::id()
        );
        let _home_guard = EnvGuard::set("HOME", Some(fake_home.as_str()));
        let _token_guard = EnvGuard::set(QWEN_OAUTH_TOKEN_ENV, None);
    ...
    fn resolve_provider_credential_opencode_go_env() {
        ...
        let _operant_guard = EnvGuard::set("OPERANT_API_KEY", None);
    ...
    fn provider_runtime_options_from_config_propagates_native_tools() {
        ...
        let mut groq = operant_config::schema::ModelProviderConfig {
            native_tools: Some(true),
            ..Default::default()
        };
    ...
    fn resilient_provider_ignores_duplicate_and_invalid_fallbacks() {
        let reliability = operant_config::schema::ReliabilityConfig {
            provider_retries: 1,
    ...
    fn resilient_provider_errors_for_invalid_primary() {
        let reliability = operant_config::schema::ReliabilityConfig::default();
        let provider = create_resilient_provider(
            "totally-invalid",
            Some("provider-test-credential"),
            None,
    ...
    fn provider_runtime_options_extra_headers_passed_through() {
        ...
        extra_headers.insert("X-Title".to_string(), "operant".to_string());
        let options = ProviderRuntimeOptions {
            extra_headers,
            ..ProviderRuntimeOptions::default()
        };
        assert_eq!(options.extra_headers.len(), 1);
        assert_eq!(options.extra_headers.get("X-Title").unwrap(), "operant");
    }

    #[test]
    fn env_provider_url_overrides_api_url() {
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("OPERANT_PROVIDER_URL", "http://env-ollama:11434") };
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::remove_var("OPERANT_PROVIDER_URL") };
    }
