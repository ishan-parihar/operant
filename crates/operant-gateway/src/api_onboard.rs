...
pub async fn handle_catalog(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let providers: Vec<CatalogProvider> = operant_providers::list_providers()
        .into_iter()
...
/// (`operant_providers::create_provider(...).list_models()`), which goes
...
pub async fn handle_catalog_models(
    ...
    let handle = match operant_providers::create_provider(&q.provider, None) {
        Ok(h) => h,
        Err(e) => {
...
pub async fn handle_sections(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let map_keyed_roots: std::collections::HashSet<&'static str> =
        operant_config::schema::Config::map_key_sections()
            .iter()
...
/// * `providers` → `operant_providers::list_providers()` (CLI's catalog).
/// * `memory` → `operant_memory::selectable_memory_backends()`.
...
fn providers_picker(cfg: &operant_config::schema::Config) -> Vec<PickerItem> {
    operant_providers::list_providers()
        .into_iter()
        .map(|p| {
...
fn memory_picker(cfg: &operant_config::schema::Config) -> Vec<PickerItem> {
    let current = cfg.memory.backend.clone();
    operant_memory::selectable_memory_backends()
        .iter()
        .map(|b| PickerItem {
            key: b.key.to_string(),
...
fn schema_walk_picker(cfg: &operant_config::schema::Config, section: &str) -> Vec<PickerItem> {
...
fn schema_walk_picker_with_none(
    cfg: &operant_config::schema::Config,
    section: &str,
...
pub async fn handle_section_select(
    ...
    let (fields_prefix, created) = match section.as_str() {
        "providers" => {
            ...
            if let Err(e) =
                operant_runtime::onboard::field_visibility::apply_provider_trait_defaults(
                    &mut working,
                    &key,
                    &prefix,
                )
...
mod tests {
    ...
    fn empty_cfg() -> operant_config::schema::Config {
    ...
    fn providers_picker_sources_from_list_providers() {
        // Single source of truth: operant_providers::list_providers().
    ...
    fn memory_picker_sources_from_selectable_backends() {
        ...
        // Mirrors operant_memory::selectable_memory_backends() exactly.
