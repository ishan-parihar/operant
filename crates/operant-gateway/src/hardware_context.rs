...
//!
//! Operant's agent loop calls [`operant_hardware::boot`] on **every** request,
//! which re-reads `~/.operant/hardware/` from disk.  Writing to those files
...
/// Return `~/.operant/hardware/` or an error string.
fn hardware_dir() -> Result<PathBuf, String> {
    directories::BaseDirs::new()
        .map(|b| b.home_dir().join(".operant").join("hardware"))
...
/// `POST /api/hardware/pin` — register a single GPIO pin assignment.
///
/// Appends one line to `~/.operant/hardware/devices/<device>.md`:
/// ```text
...
///
/// Since [`operant_hardware::boot`] re-reads from disk on every agent invocation,
...
pub async fn handle_hardware_reload(
    ...
    let context = operant_hardware::load_hardware_context_prompt(&[]);
