...
pub struct CronAddBody {
    ...
    pub delivery: Option<operant_runtime::cron::DeliveryConfig>,
...
fn cron_schedule_from_api(
    expr: String,
    tz: Option<String>,
) -> Result<operant_runtime::cron::Schedule, (StatusCode, Json<serde_json::Value>)> {
    let schedule = operant_runtime::cron::Schedule::Cron { expr, tz };
    operant_runtime::cron::validate_schedule(&schedule, chrono::Utc::now())
...
pub async fn handle_api_status(
    ...
    let health = operant_runtime::health::snapshot();
    ...
    let git_sha = option_env!("GIT_COMMIT_SHA").unwrap_or("unknown");
    let git_dirty = option_env!("GIT_DIRTY").unwrap_or("false") == "true";
...
pub async fn handle_api_cron_list(
    ...
    let config = state.config.lock().clone();
    match operant_runtime::cron::list_jobs(&config) {
...
pub async fn handle_api_cron_get(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path(id): Path<String>,
) -> impl IntoResponse {
    ...
    let config = state.config.lock().clone();
    match operant_runtime::cron::get_job(&config, &id) {
...
pub async fn handle_api_cron_add(
    ...
    if let Err(e) = operant_runtime::cron::validate_delivery_config(delivery.as_ref()) {
        return (
            StatusCode::BAD_REQUEST,
    ...
    let result = if is_agent {
        ...
        let session_target = session_target
            .as_deref()
            .map(operant_runtime::cron::SessionTarget::parse)
            .unwrap_or_default();

        let default_delete = matches!(schedule, operant_runtime::cron::Schedule::At { .. });
        let delete_after_run = delete_after_run.unwrap_or(default_delete);

        operant_runtime::cron::add_agent_job(
            &config,
            name,
            schedule,
            prompt,
        ...
        operant_runtime::cron::add_shell_job_with_approval(
...
pub async fn handle_api_cron_runs(
    ...
    if let Err(e) = operant_runtime::cron::get_job(&config, &id) {
        return (
            StatusCode::NOT_FOUND,
            Json(serde_json::json!({"error": format!("Cron job not found: {e}")})),
        )
    ...
    match operant_runtime::cron::list_runs(&config, &id, limit) {
        Ok(runs) => {
...
pub async fn handle_api_cron_run(
    ...
    let job = match operant_runtime::cron::get_job(&config, &id) {
        Ok(job) => job,
        Err(e) => {
            return (
                StatusCode::NOT_FOUND,
    ...
    let (mut success, output) = operant_runtime::cron::scheduler::execute_job_now(
        &config,
        &job,
    ...
    if job.delivery.mode.eq_ignore_ascii_case("announce")
        ...
        && let Err(e) = operant_runtime::cron::scheduler::deliver_announcement(
            &config,
            channel,
            target,
    ...
    if let Err(e) = operant_runtime::cron::record_run(
        &config,
        &job.id,
        started_at,
        finished_at,
    ...
    if let Err(e) =
        operant_runtime::cron::record_last_run(&config, &job.id, finished_at, success, &output)
    {
        tracing::warn!(
            job_id = %job.id,
            error = %e,
...
pub async fn handle_api_cron_patch(
    ...
    let existing = match operant_runtime::cron::get_job(&config, &id) {
        Ok(j) => j,
        Err(e) => {
            return (
                StatusCode::NOT_FOUND,
        ...
        let (expr, existing_tz) = match (&existing.schedule, new_expr) {
            (_, Some(expr)) => {
                let existing_tz = match &existing.schedule {
                    operant_runtime::cron::Schedule::Cron { tz, .. } => tz.clone(),
                    _ => None,
                };
                (expr, existing_tz)
            }
            (operant_runtime::cron::Schedule::Cron { expr, tz }, None) => {
                (expr.clone(), tz.clone())
            }
            (_, None) => {
    ...
    let is_agent = matches!(existing.job_type, operant_runtime::cron::JobType::Agent);
    ...
    let patch = operant_runtime::cron::CronJobPatch {
        name,
        schedule,
        command: patch_command,
        prompt: patch_prompt,
        ..operant_runtime::cron::CronJobPatch::default()
    };

    match operant_runtime::cron::update_shell_job_with_approval(&config, &id, patch, false) {
        Ok(job) => Json(serde_json::json!({"status": "ok", "job": job})).into_response(),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn handle_api_cron_delete(
    ...
    let config = state.config.lock().clone();
    match operant_runtime::cron::remove_job(&config, &id) {
...
pub async fn handle_api_integrations(
    ...
    let config = state.config.lock().clone();
    let entries = operant_runtime::integrations::registry::all_integrations(&config);
...
pub async fn handle_api_integrations_settings(
    ...
    for entry in &entries {
        let enabled = matches!(
            entry.status,
            operant_runtime::integrations::IntegrationStatus::Active
        );
        settings.insert(
            entry.name.clone(),
            serde_json::json!({
...
pub async fn handle_api_memory_list(
    ...
    if params.query.is_some() || params.since.is_some() || params.until.is_some() {
        ...
        let category = params.category.as_deref().map(|cat| match cat {
            "core" => operant_memory::MemoryCategory::Core,
            "daily" => operant_memory::MemoryCategory::Daily,
            "conversation" => operant_memory::MemoryCategory::Conversation,
            other => operant_memory::MemoryCategory::Custom(other.to_string()),
        });
...
pub async fn handle_api_memory_store(
    ...
    let category = body
        ...
        .unwrap_or(operant_memory::MemoryCategory::Core);
...
pub async fn handle_api_cli_tools(
    ...
    let tools = operant_tools::cli_discovery::discover_cli_tools(&[], &[]);
...
pub async fn handle_api_health(
    ...
    let snapshot = operant_runtime::health::snapshot();
...
pub async fn handle_api_session_message_post(
    ...
    let message = operant_providers::ChatMessage::assistant(&body.content);
...
pub async fn handle_claude_code_hook(
    State(state): State<AppState>,
    Json(payload): Json<operant_tools::claude_code_runner::ClaudeCodeHookEvent>,
) -> impl IntoResponse {
...
mod tests {
    ...
    fn test_state(config: operant_config::schema::Config) -> AppState {
        use crate::{
            GatewayRateLimiter, IdempotencyStore, auth_rate_limit::AuthRateLimiter,
            nodes::NodeRegistry, session_queue::SessionActorQueue, sse::EventBuffer,
        };
    ...
    async fn session_message_post_persists_and_broadcasts_to_session() {
        ...
        backend
            .append(
                "gw_operator-1",
                &operant_providers::ChatMessage::assistant("existing"),
            )
            .unwrap();
    ...
    async fn cron_api_accepts_agent_jobs() {
        ...
        let config = state.config.lock().clone();
        let jobs = operant_runtime::cron::list_jobs(&config).unwrap();
        assert_eq!(jobs.len(), 1);
        assert_eq!(jobs[0].job_type, operant_runtime::cron::JobType::Agent);
    ...
    async fn cron_api_timezone_add_persists_explicit_timezone() {
        ...
        assert_eq!(
            jobs[0].schedule,
            operant_runtime::cron::Schedule::Cron {
                expr: "0 9 * * *".to_string(),
    ...
    async fn cron_api_timezone_patch_schedule_preserves_existing_timezone() {
        ...
        std::fs::create_dir_all(&config.workspace_dir).unwrap();
        let state = test_state(config);
        let job = operant_runtime::cron::add_shell_job_with_approval(
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let updated = operant_runtime::cron::get_job(&state.config.lock().clone(), &job.id)
            .expect("updated job");
        assert_eq!(
            updated.schedule,
            operant_runtime::cron::Schedule::Cron {
    ...
    async fn cron_api_rejects_announce_delivery_without_target() {
        ...
        let config = state.config.lock().clone();
        assert!(
            operant_runtime::cron::list_jobs(&config)
                .unwrap()
                .is_empty()
        );
    }
    ...
    async fn cron_api_run_executes_shell_job_and_records_run() {
        ...
        let runs = operant_runtime::cron::list_runs(&state.config.lock().clone(), &job.id, 10)
            .expect("runs listed");
