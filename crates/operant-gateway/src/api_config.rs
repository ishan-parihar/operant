...
use operant_config::api_error::{ConfigApiCode, ConfigApiError};
use operant_runtime::onboard::{Section, field_visibility};
...
pub struct PatchResponse {
    ...
    pub warnings: Vec<operant_config::validation_warnings::ValidationWarning>,
}
...
fn prop_kind_wire(kind: operant_config::traits::PropKind) -> &'static str {
    use operant_config::traits::PropKind;
    match kind {
...
// Typed-value coercion lives in `operant_config::typed_value` — both the
...
use operant_config::typed_value::coerce_for_set_prop as json_to_setprop_string;
...
fn lookup_prop_field(
    ...
    path: &str,
) -> Option<operant_config::traits::PropFieldInfo> {
    config
        .prop_fields()
        .into_iter()
...
async fn persist_and_swap(
    state: &AppState,
    new_config: operant_config::schema::Config,
...
pub async fn compute_drift(in_memory: &operant_config::schema::Config) -> Vec<DriftEntry> {
    ...
    let on_disk: operant_config::schema::Config =
        match toml::from_str::<operant_config::schema::Config>(&raw) {
            Ok(mut cfg) => {
    ...
    let in_memory_props: std::collections::HashMap<String, operant_config::traits::PropFieldInfo> =
        in_memory
            .prop_fields()
            .into_iter()
            .map(|p| (p.name.clone(), p))
            .collect();
    let on_disk_props: std::collections::HashMap<String, operant_config::traits::PropFieldInfo> =
        on_disk
            .prop_fields()
            .into_iter()
            .map(|p| (p.name.clone(), p))
...
pub async fn handle_prop_put(
    ...
    if let Some(comment) = body.comment.as_ref() {
        ...
        if let Err(e) =
            operant_config::comment_writer::apply_comments(&config_path, &annotations).await
        {
...
pub async fn handle_templates(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let templates: Vec<TemplateEntry> = operant_config::schema::Config::map_key_sections()
        .into_iter()
        .map(|s| TemplateEntry {
            path: s.path,
            kind: match s.kind {
                operant_config::traits::MapKeyKind::Map => "map",
                operant_config::traits::MapKeyKind::List => "list",
            },
...
pub async fn handle_migrate(State(state): State<AppState>, headers: HeaderMap) -> Response {
    ...
    let migrated = match operant_config::migration::migrate_file(&raw) {
        Ok(out) => out,
        Err(e) => {
            return error_response(ConfigApiError::new(
                ConfigApiCode::ValidationFailed,
    ...
    match migrated {
        Some(new_content) => {
            ...
            let new_cfg: operant_config::schema::Config = match toml::from_str(&new_content) {
                Ok(c) => c,
                Err(e) => {
                    return error_response(ConfigApiError::new(
                        ConfigApiCode::ReloadFailed,
            ...
            *state.config.lock() = new_cfg;
            ...
            axum::Json(MigrateResponse {
                ...
                schema_version: operant_config::migration::CURRENT_SCHEMA_VERSION,
            })
            .into_response()
        }
...
pub async fn handle_options_prop(
    ...
    response.headers_mut().insert(
        header::ETAG,
        HeaderValue::from_str(etag).expect("etag is a content-hash string, valid header value"),
    );
    response
}
...
fn schema_body_value() -> serde_json::Value {
    let schema = schemars::schema_for!(operant_config::schema::Config);
...
mod tests {
    ...
    // typed-value coercion tests live in operant_config::typed_value
    // — shared helper, single source of truth.
    //
    // build_comment_prefix tests live in operant_config::comment_writer
    // — same reason.
    ...
    fn test_op_coercion_bool_typed_value_matches_stored() {
        let mut cfg = operant_config::schema::Config::default();
    ...
    fn collect_warnings_empty_when_fallback_unset() {
        ...
        // gets caught.
        let cfg = operant_config::schema::Config::default();
        let warnings = cfg.collect_warnings();
        assert!(
            !warnings
                .iter()
    ...
    async fn compute_drift_returns_empty_when_in_memory_matches_disk() {
        ...
        let cfg = operant_config::schema::Config {
    ...
    async fn compute_drift_surfaces_mismatched_non_secret_field() {
        let (_tmp, path) = temp_config_path();
        let mut cfg = operant_config::schema::Config {
            config_path: path.clone(),
            ..Default::default()
        };
    ...
    async fn apply_comments_writes_decoration_to_existing_value() {
        ...
        operant_config::comment_writer::apply_comments(
            &path,
    ...
    fn scrub_credentials_catches_credential_shaped_strings() {
        ...
        use operant_runtime::agent::loop_::scrub_credentials;
