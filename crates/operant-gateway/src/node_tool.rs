//! Wraps a node capability as a operant [`Tool`] so it can be dispatched
...
use operant_tools::node_capabilities::requires_approval;

/// Default timeout for node invocations (30 seconds).
const NODE_INVOKE_TIMEOUT_SECS: u64 = 30;

/// A operant [`Tool`] backed by a node capability.
///
