...
impl EventBuffer {
    ...
    pub fn push(&self, event: serde_json::Value) {
        let mut buf = self
            .inner
            .lock()
            .expect("sse buffer mutex poisoned — programmer error");
...
impl operant_runtime::observability::Observer for BroadcastObserver {
    fn record_event(&self, event: &operant_runtime::observability::ObserverEvent) {
        ...
        let json = match event {
            operant_runtime::observability::ObserverEvent::LlmRequest {
                provider, model, ..
            } => serde_json::json!({
                "type": "llm_request",
                "provider": provider,
            ...
            operant_runtime::observability::ObserverEvent::ToolCall {
                tool,
                duration,
                success,
            } => serde_json::json!({
            ...
            operant_runtime::observability::ObserverEvent::ToolCallStart { tool, .. } => {
                serde_json::json!({
            ...
            operant_runtime::observability::ObserverEvent::TurnComplete => {
                serde_json::json!({
                    "type": "turn_complete",
                    "timestamp": chrono::Utc::now().to_rfc3339(),
                })
            }
            operant_runtime::observability::ObserverEvent::ChannelMessage {
                channel,
                direction,
            } => serde_json::json!({
            ...
            operant_runtime::observability::ObserverEvent::Error { component, message } => {
                serde_json::json!({
                    "type": "error",
                    "component": component,
                    "message": message,
            ...
            operant_runtime::observability::ObserverEvent::AgentStart { provider, model } => {
                serde_json::json!({
                    "type": "agent_start",
                    "provider": provider,
                    "model": model,
            ...
            operant_runtime::observability::ObserverEvent::AgentEnd {
                provider,
                model,
                duration,
                tokens_used,
            ...
            operant_runtime::observability::ObserverEvent::CacheHit {
                cache_type,
                tokens_saved,
            } => serde_json::json!({
                "type": "cache_hit",
            ...
            operant_runtime::observability::ObserverEvent::CacheMiss { cache_type } => {
                serde_json::json!({
                    "type": "cache_miss",
                    "cache_type": cache_type,
                    "timestamp": chrono::Utc::now().to_rfc3339(),
...
mod tests {
    use super::*;
    use operant_runtime::observability::{Observer, ObserverEvent};
    ...
    fn factory_observer_events_reach_broadcast_hook() {
        ...
        operant_runtime::observability::clear_broadcast_hook();

        let (tx, mut rx) = tokio::sync::broadcast::channel(16);
        let buffer = Arc::new(EventBuffer::new(16));
        let bo: Arc<dyn Observer> = Arc::new(BroadcastObserver::new(tx, buffer.clone()));
        operant_runtime::observability::set_broadcast_hook(bo);

        // Same factory call site as `process_message` in the agent loop.
        let cfg = operant_config::schema::ObservabilityConfig {
            backend: "noop".into(),
            ..Default::default()
        };
        let observer = operant_runtime::observability::create_observer(&cfg);
