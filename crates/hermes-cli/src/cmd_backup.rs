...
async fn cmd_create(_config: &AppConfig, output: Option<PathBuf>) -> Result<()> {
    ...
    let data_dir = hermes_core::platform::hermes_data_dir();
    if data_dir.exists() {
...
async fn cmd_list(_config: &AppConfig) -> Result<()> {
    ...
    for entry in fs::read_dir(&cwd)? {
        ...
        if name_str.starts_with("hermes_backup_") && entry.file_type()?.is_dir() {
