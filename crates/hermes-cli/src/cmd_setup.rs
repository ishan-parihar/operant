...
fn step_api_key(config: &mut AppConfig, provider_key: &str, provider_name: &str) -> Result<()> {
    ...
    if let Some(key) = &config.client.api_key {
        ...
        if !key.is_empty() {
            ...
            print_warning("No API key set. You can set it later via 'hermes auth'.");
        }
    }
...
fn is_gateway_enabled(settings: &GatewaySettings, platform_key: &str) -> bool {
    match platform_key {
        ...
        "mattermost" => settings.mattermost_enabled,
        ...
        "feishu_lark" => settings.feishu_lark_enabled,
        ...
        "microsoft_teams" => settings.microsoft_teams_enabled,
        "facebook_messenger" => settings.facebook_messenger_enabled,
        "wechat" => settings.wechat_enabled,
        "viber" => settings.viber_enabled,
        "google_business_messages" => settings.google_business_messages_enabled,
        "twitter" => settings.twitter_enabled,
...
async fn step_gateway(config: &mut AppConfig, _reconfigure: bool) -> Result<bool> {
    ...
    if changed {
        print_warning("Gateway configuration changed. You may need to restart Hermes for changes to take effect.");
    }
...
async fn step_terminal(config: &mut AppConfig, _reconfigure: bool) -> Result<()> {
    ...
    print_info("Customise how Hermes behaves in the terminal.");
...
fn persist_config(config: &AppConfig) -> Result<()> {
    ...
    let config_path = paths
        ...
        .unwrap_or_else(|| {
            dirs::home_dir()
                .map(|h| h.join(".hermes").join("hermes.toml"))
                .unwrap_or_else(|| PathBuf::from("hermes.toml"))
        });
