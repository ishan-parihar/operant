...
//!
//! Mirrors sections from `hermes-agent/hermes_cli/doctor.py`:
...
use hermes_core::config::{AppConfig, TerminalBackend};
use hermes_core::platform::{find_node, hermes_home};
...
/// Return the hermes-rs project root (parent of `crates/hermes-cli/`).
fn project_root() -> &'static Path {
    // CARGO_MANIFEST_DIR = .../hermes-rs/crates/hermes-cli
    // parent = crates/, parent = hermes-rs/
    static ROOT: std::sync::OnceLock<PathBuf> = std::sync::OnceLock::new();
    ROOT.get_or_init(|| {
        Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .expect("hermes-cli/Cargo.toml has a parent")
            .parent()
...
pub fn run_platform_checks(
    ...
    for (name, required_envs, is_configured) in &tool_categories {
        ...
        if available {
            ...
            let missing: Vec<&str> = required_envs
                ...
                .collect();
            if missing.is_empty() {
                check_warn(name, "(not configured — run hermes setup)");
            } else {
    ...
    if any_unavailable {
        issues.push(
            "Run 'hermes setup' to configure missing API keys for full tool access".to_string(),
        );
    }
    ...
    if hub_dir.exists() {
        ...
        issues.push("Initialize skills hub with 'hermes skills list'".to_string());
    }
        ...
        Some("honcho") => {
            ...
            if let Some(_key) = honcho_key {
                ...
                check_fail(
                    ...
                    "(set HONCHO_API_KEY in .env or run hermes memory setup)",
                );
                issues.push(
    ...
    if profiles_dir.exists() {
        ...
        if entries.is_empty() {
            ...
            let wrapper_dir = hermes_home().join("bin");
            if wrapper_dir.is_dir() {
                if let Ok(wrapper_entries) = std::fs::read_dir(&wrapper_dir) {
                    for wrapper in wrapper_entries.flatten() {
                        ...
                        // Simple heuristic: check if it's a hermes profile wrapper
...
fn detect_memory_provider(config: &AppConfig) -> Option<String> {
    // Check if there's a hermes.toml we can parse for memory.provider.
    // The AppConfig doesn't carry a memory.provider field directly,
    // so we attempt to read the TOML file from config paths.
    let config_paths = [
        project_root().join("hermes.toml"),
        hermes_home().join("hermes.toml"),
    ];
