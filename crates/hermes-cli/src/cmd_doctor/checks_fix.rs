//! Auto-fix implementation for `hermes doctor --fix`.
//!
//! Mirrors the `--fix` logic from `hermes-agent/hermes_cli/doctor.py`:
//! - Create missing directories (hermes_home, config, data, memories, sessions, logs, skills, cron)
...
use hermes_core::platform::{
    hermes_config_dir, hermes_data_dir, hermes_home, hermes_memories_dir, hermes_sessions_dir,
    hermes_skills_dir,
};
use std::path::Path;
...
pub async fn cmd_fix(config: &AppConfig) -> Result<()> {
    ...
    let dirs: &[(fn() -> std::path::PathBuf, &str)] = &[
        ...
        (hermes_config_dir, "HERMES_HOME/config"),
        (hermes_data_dir, "HERMES_HOME/data"),
    ...
    if !env_path.exists() {
        match std::fs::write(&env_path, "") {
            Ok(()) => {
                ...
                println!("         Run 'hermes setup' to configure API keys");
                fixed += 1;
            }
            Err(e) => {
...
";
    ...
    if let Ok(exe_path) = std::env::current_exe() {
        println!("  \u{2713} hermes binary: {}", exe_path.display());
    }
