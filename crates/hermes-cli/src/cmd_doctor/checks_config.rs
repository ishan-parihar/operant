//! Config, auth, and directory-structure checks for `hermes doctor`.
//!
...
//! - Directory Structure (hermes_home, subdirs, SOUL.md, memories, state.db, WAL)
...
/// Display-friendly representation of `HERMES_HOME` (e.g. `~/.hermes`).
...
pub fn run_config_checks(config: &AppConfig, issues: &mut Vec<String>) {
    ...
    let toml_path = hh.join("hermes.toml");
    ...
    if toml_path.exists() {
        check_ok(&format!("{}/hermes.toml exists", dhh), "");
    }
    if !yaml_path.exists() && !toml_path.exists() {
        check_warn(
            "No config file found",
            &format!("(expected {}/config.yaml or hermes.toml)", dhh),
        );
    }
            ...
            if let Some(pdef) = by_name {
                ...
                issues.push(
                    "client.base_url does not match a known provider. ".to_string()
                        + "Run 'hermes setup' to configure a supported provider.",
                );
            }
        }
    } else {
        check_warn("client.base_url is not configured", "");
        issues.push("Run 'hermes setup' to configure a provider and base URL".to_string());
    }
    ...
    if !set_providers.is_empty() {
        ...
        issues.push(
            "No provider API keys configured. Run 'hermes setup' or set the appropriate *_API_KEY in .env"
                .to_string(),
        );
    }
        ...
        match std::fs::metadata(&wal_path) {
            Ok(meta) => {
                ...
                if wal_size > 50 * 1024 * 1024 {
                    ...
                    issues.push(
                        "Large WAL file — run 'hermes doctor --fix' to checkpoint".to_string(),
                    );
