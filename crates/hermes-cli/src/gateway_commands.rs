...
/// runtime state needed for full command parity with hermes-agent Python.
...
/// registry via [`hermes_core::plugins::handle_plugin_command`].
pub fn handle_command(cmd_name: &str, _args: &str, ctx: &CommandContext<'_>) -> Option<String> {
    ...
    let def = match def {
        ...
        None => {
            ...
            return hermes_core::plugins::handle_plugin_command(cmd_name, _args);
        }
    };
    ...
    Some(match def.name {
        ...
        "session" => {
            ...
            if let Some(gateway) = ctx.gateway {
                ...
                lines.push("• Gateway not connected.".to_string());
                lines.push("• Use CLI: `hermes sessions list`".to_string());
            }
            lines.join("\n")
        }
        ...
        "restart" => {
            ...
            if let Some(gateway) = ctx.gateway {
                ...
                msg.push_str("\nGateway not connected — use CLI `hermes gateway restart`.");
            }
            msg
        }
        ...
        "sessions" => {
            ...
            if let Some(gateway) = ctx.gateway {
                ...
                lines.push("• Gateway not connected. Use CLI `hermes sessions list`.".to_string());
            }
            lines.join("\n")
        }
        ...
        "resume" => {
            let a = _args.trim();
            if a.is_empty() {
                "Usage: `/resume <session-id>` — resumes a saved session by ID.\nFind session IDs via `/sessions` or CLI `hermes sessions list`."
                    .into()
            } else if let Some(gateway) = ctx.gateway {
                let store = gateway.get_session_store();
                if let Some(session) = store.get_session(a) {
                    format!(
                        "✅ Session found: `{}`\n• Platform: {}\n• User: {}\n• Last active: {}\n\nResume via CLI: `hermes sessions resume {}`",
                        session.session_id, session.platform, session.platform_user_id, session.last_active, session.hermes_session_id
                    )
                } else {
                    format!("❌ No session found with ID: `{}`. Use `/sessions` to list active sessions.", a)
                }
            } else {
                format!("Session resume requested for `{}`. Use CLI `hermes sessions resume` to resume sessions.", a)
            }
        }
        ...
        "profile" => {
            ...
            format!(
                "*Active Profile:*\n• Model: `{}`\n• Max iterations: `{}`\n• Reasoning: `{}`\n\nUse CLI `hermes profile` for full profile management.",
        ...
        "sethome" => {
            if let Some(gateway) = ctx.gateway {
                ...
                match dir.register_channel(
                    ...
                    ctx.platform,
                    Some(ctx.user_id),
                    hermes_core::gateway::ChannelType::Direct,
                    if is_admin {
                    ...
                    Err(e) => format!(
                        "⚠️ Channel already registered: {}\nUse CLI `hermes channels list` for details.",
                        e
                    ),
                }
            } else {
        ...
        "personality" => {
            ...
            if a.is_empty() {
                ...
                msg.push_str("\nUsage: `/personality <name>` — select an AI personality.\nUse CLI `hermes personality list` to see available personalities.");
                msg
            } else {
                if let Some(gateway) = ctx.gateway {
                    let store = gateway.get_session_store();
        ...
        "reload-mcp" => {
            ...
            if let Some(gateway) = ctx.gateway {
                ...
                msg.push_str("\nGateway not connected. Use CLI `hermes mcp restart`.");
            }
            msg
        }
        ...
        "reload-skills" => {
            ...
            if let Some(gateway) = ctx.gateway {
                ...
                msg.push_str("\nGateway not connected. Use CLI `hermes skills reload`.");
            }
            msg
        }
        ...
        "skills" => {
            ...
            if skills_dir.exists() && skills_dir.is_dir() {
                ...
                if skill_names.is_empty() {
                    "📦 No skills found in the skills directory.\nUse CLI `hermes skills list` or `hermes skills install` to manage skills."
                        .into()
                } else {
                    skill_names.sort();
                ...
                format!(
                    "📦 Skills directory not found: `{}`.\nUse CLI `hermes skills list` to see available skills.",
                    skills_dir.display()
                )
            }
        }
        ...
        "curator" => {
            ...
            match a {
                ...
                "pin" | "unpin" | "archive" | "list-archived" | "pause" | "resume" | "restore" => {
                    format!(
                        "🔧 Curator `{}` command received.\nFull curator management via CLI: `hermes curator {}`",
                        a, a
                    )
                }
        ...
        "usage" => {
            ...
            if let Some(gateway) = ctx.gateway {
                ...
                lines.push("• Gateway not connected. Use CLI `hermes insights sessions`.".to_string());
            }
            lines.join("\n")
        }
        ...
        "insights" => {
            let a = _args.trim();
            if a.is_empty() {
                "📈 Session insights are available via CLI.\nUse `hermes insights sessions` to view session analytics."
                    .into()
            } else {
                format!(
                    "📈 Insights for the last `{}` days: available via CLI `hermes insights sessions --days {}`.",
                    a, a
                )
            }
        }
        ...
        "debug" => {
            ...
            lines.push("For full debug report, use CLI: `hermes debug share`".to_string());
            lines.join("\n")
        }
        ...
        "update" => {
            ...
            format!(
                "📦 Hermes is at version **v{}**.\nCheck for updates via CLI: `hermes update check`\nSee what's new: `hermes changelog`",
                version
            )
        }
        ...
        "platform" => {
            ...
            match a {
                ...
                _ => format!(
                    "Platform `{}` command received.\nFull platform management via CLI: `hermes gateway`",
                    a
                ),
            }
        }
...
///
/// Plugin commands registered via [`hermes_core::plugins::register_plugin_command`]
...
pub fn telegram_bot_commands() -> String {
    ...
    for cmd in hermes_core::plugins::get_plugin_commands() {
        if !first {
            json.push(',');
        }
        first = false;
