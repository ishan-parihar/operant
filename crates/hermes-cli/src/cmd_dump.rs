...
pub async fn handle_dump_command(config: &AppConfig, all: bool) -> Result<()> {
    if all {
        ...
        println!("Model       : {}", if model.is_empty() { "not set" } else { model });
        let key_ok = config.client.api_key.as_ref().map_or(false, |k| !k.is_empty());
        println!("API key     : {}", if key_ok { "configured" } else { "not set" });
        ...
        if gateway.telegram_enabled { gateways.push("telegram"); }
        if gateway.discord_enabled { gateways.push("discord"); }
        if gateway.slack_enabled { gateways.push("slack"); }
        println!("Gateways    : {}", if gateways.is_empty() { "none enabled".to_string() } else { gateways.join(", ") });
        println!("{:─<60}", "");
