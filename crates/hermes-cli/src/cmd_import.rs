...
async fn cmd_import(_config: &AppConfig, path: &PathBuf, force: bool) -> Result<()> {
    ...
    if !config_backup.exists() && !data_backup.exists() {
        anyhow::bail!(
            "No Hermes backup found at {}. Expected 'config/' or 'data/' subdirectories.",
            path.display()
        );
    }
    ...
    if config_backup.exists() {
        let dst = hermes_core::platform::hermes_config_dir();
        copy_dir_recursive(&config_backup, &dst)?;
        println!("Restored config from {}", config_backup.display());
        restored += 1;
    }
    if data_backup.exists() {
        let dst = hermes_core::platform::hermes_data_dir();
