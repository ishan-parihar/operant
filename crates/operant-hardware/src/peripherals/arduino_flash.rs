//! Flash Operant Arduino firmware via arduino-cli.
//!
...
/// Operant Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
...
/// Flash Operant firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("operant_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling Operant Arduino firmware...");
    ...
    println!("Operant firmware flashed successfully.");
