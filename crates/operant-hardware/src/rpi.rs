...
impl RpiSystemContext {
    ...
    /// Write an `rpi0.md` hardware context file to `~/.operant/hardware/devices/`.
    ...
    pub fn write_hardware_context_file(&self) {
        ...
        let devices_dir = home.join(".operant").join("hardware").join("devices");
    ...
    fn device_profile_markdown(&self) -> String {
        ...
        let _ = writeln!(
            s,
            "- Runtime: Operant native (rppal — no serial, no mpremote)"
        );
        if let Some(ref iface) = self.wifi_interface {
            let _ = writeln!(s, "- WiFi interface: {}", iface);
        }
