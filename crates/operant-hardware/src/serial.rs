...
/// Default baud rate for Operant serial devices.
...
/// Serial transport for Operant hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify Operant firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "operant"`.
    ///
    /// Returns `true` if a Operant device responds, `false` otherwise.
    ...
    pub async fn ping_handshake(&self) -> bool {
        ...
        match result {
            Ok(Ok(resp)) => {
                // Accept if firmware field is "operant" (in data or top-level)
                resp.ok
                    && resp
                        .data
                        .get("firmware")
