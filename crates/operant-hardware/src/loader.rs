//! Plugin manifest loader — scans `~/.operant/tools/` at startup.
//!
...
/// Scan `~/.operant/tools/` and return all valid plugins.
///
...
pub fn scan_plugin_dir() -> Vec<LoadedPlugin> {
    ...
    println!(
        ...
        match dirs_home().as_deref().filter(|s| !s.is_empty()) {
            Some(home) => tools_dir
                .to_str()
                .unwrap_or("~/.operant/tools")
                .replace(home, "~"),
            None => tools_dir.to_str().unwrap_or("~/.operant/tools").to_string(),
        }
    );
...
"#;
    ...
    fn scan_plugin_dir_skips_broken_manifests_without_panicking() {
        ...
        // always uses ~/.operant/tools), but we can verify load_one_plugin
    ...
    #[test]
    fn plugin_tools_dir_returns_path_ending_in_operant_tools() {
        let path = plugin_tools_dir().expect("should resolve");
        let display = path.to_string_lossy();
        let expected = std::path::Path::new(".operant").join("tools");
