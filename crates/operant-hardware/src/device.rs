...
pub enum DeviceKind {
    ...
    Nucleo,
    /// Unknown VID that passed the Operant firmware ping handshake.
    Generic,
...
impl DeviceRegistry {
    ...
    ///    if the device responds with Operant firmware.
        ...
        for info in scan_serial_devices() {
            ...
            let probe_transport = if !is_known_vid {
                ...
                if !probe.ping_handshake().await {
                    tracing::debug!(
                        port = %info.port_path,
                        "skipping unknown device: no Operant firmware response"
                    );
                    continue;
                }
                Some(probe)
...
impl DeviceRegistry {
    ...
    /// Operant firmware is alive, and re-attaches the transport.
    ///
