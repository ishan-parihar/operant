...
//!    `~/.operant/hardware/datasheets/<device>.pdf`.
...
/// Manages device datasheet files in `~/.operant/hardware/datasheets/`.
...
impl DatasheetManager {
    /// Create a manager rooted at the default Operant datasheets directory.
    pub fn new() -> Option<Self> {
        let home = directories::BaseDirs::new()?.home_dir().to_path_buf();
        Some(Self {
            datasheet_dir: home.join(".operant").join("hardware").join("datasheets"),
        })
    }
    ...
    ///
    /// The file is saved as `~/.operant/hardware/datasheets/<device_name>.pdf`.
    ...
    pub async fn download_datasheet(
        ...
        let client = reqwest::Client::builder()
            .user_agent("Operant/0.1 (datasheet downloader)")
...
impl Default for DatasheetManager {
    fn default() -> Self {
        Self::new().unwrap_or_else(|| Self {
            datasheet_dir: PathBuf::from(".operant/hardware/datasheets"),
        })
    }
}
        ...
        match action.as_str() {
                    ...
                    Ok(path) => Ok(ToolResult {
                        ...
                        output: format!(
                            "Datasheet for '{device}' downloaded successfully.\n\
                             ...
                             ~/.operant/hardware/devices/aardvark0.md with the key \
            ...
            "list" => {
                ...
                let output = if datasheets.is_empty() {
                    ...
                    format!(
                        "{} cached datasheet(s) in ~/.operant/hardware/datasheets/:\n{}",
                        datasheets.len(),
                        datasheets
                            .iter()
                            .map(|n| format!("  - {n}"))
