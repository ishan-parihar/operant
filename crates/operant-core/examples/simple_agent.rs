//! Simple Operant-RS example
//!
//! This example demonstrates basic usage of the operant-core library.
...
use operant_core::{
    ...
    agent::{AgentConfig, OperantAgent},
    client::{ClientConfig, OpenAIClient},
    database::Database,
    error::Result,
    schema::ToolSchema,
    tools::{OperantTool, ToolContext, ToolRegistry, ToolResult},
};
...
async fn main() -> Result<()> {
    ...
    println!("Operant-RS Simple Agent Example\n");
    println!("This example demonstrates how to use the operant-core library.\n");
...
Use the echo tool to repeat information and the calculate tool for math."
    ...
    let agent = OperantAgent::new(
        agent_config,
        Box::new(OpenAIModelClient::new(client)),
        registry,
        database,
    );
