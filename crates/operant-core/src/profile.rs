//! Profile management for multiple isolated Operant instances.
//!
//! Each profile is a fully independent HERMES_HOME directory with its own
//! config, memory, sessions, skills, and logs. Profiles live under
//! `~/.operant/profiles/<name>/` by default.
//!
//! The "default" profile is `~/.operant` itself — backward compatible,
...
const RESERVED_NAMES: &[&str] = &["operant", "default", "test", "tmp", "root", "sudo"];
...
pub fn set_operant_home_override(path: impl Into<PathBuf>) -> OperantHomeToken {
    let previous = HERMES_HOME_OVERRIDE.with(|cell| cell.replace(Some(path.into())));
    OperantHomeToken(previous)
}

/// Reset the HERMES_HOME override using a token from `set_operant_home_override`.
pub fn reset_operant_home_override(token: OperantHomeToken) {
    HERMES_HOME_OVERRIDE.with(|cell| cell.replace(token.0));
}

/// Token for restoring HERMES_HOME override.
pub struct OperantHomeToken(Option<PathBuf>);

/// Get the current HERMES_HOME with profile awareness.
///
/// Resolution order:
/// 1. Thread-local override (from `set_operant_home_override`)
/// 2. `HERMES_HOME` environment variable
/// 3. Default: `~/.operant`
pub fn get_operant_home() -> PathBuf {
...
/// In Docker/custom deployments where HERMES_HOME points outside `~/.operant`,
/// returns HERMES_HOME directly.
pub fn get_default_operant_root() -> PathBuf {
    let native_home = platform::operant_home();
...
pub fn get_profiles_root() -> PathBuf {
    get_default_operant_root().join("profiles")
}

/// Get the active profile file path.
fn get_active_profile_path() -> PathBuf {
    get_default_operant_root().join("active_profile")
}
...
pub fn validate_profile_name(name: &str) -> Result<()> {
    ...
    if RESERVED_NAMES.contains(&name) {
        return Err(Error::Config(format!(
            "Profile name '{}' is reserved — it collides with the Operant installation itself.",
            name
        )));
    }
...
fn read_config_model(profile_dir: &Path) -> Option<String> {
    let config_path = profile_dir.join("operant.toml");
...
pub fn list_profiles() -> Result<Vec<ProfileInfo>> {
    ...
    // Default profile
    let default_home = get_default_operant_root();
...
pub fn create_profile(name: &str, clone_from: Option<&str>) -> Result<PathBuf> {
    ...
    if canon == "default" {
        return Err(Error::Config(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.operant)."
                .to_string(),
        ));
    }
    ...
    if let Some(source) = source_dir {
        for filename in &["operant.toml", ".env", "SOUL.md"] {
...
pub fn delete_profile(name: &str) -> Result<PathBuf> {
    ...
    if canon == "default" {
        return Err(Error::Config(
            "Cannot delete the default profile (~/.operant).".to_string(),
        ));
    }
...
mod tests {
    ...
    fn test_validate_profile_name() {
        ...
        assert!(validate_profile_name("operant").is_err());
    ...
    fn test_clone_profile() {
        ...
        fs::write(source_dir.join("operant.toml"), config_content).unwrap();
        ...
        let target_config = fs::read_to_string(target_dir.join("operant.toml")).unwrap();
    ...
    #[test]
    fn test_get_operant_home_override() {
        let _guard = env_lock().lock().unwrap();
        let original = env::var("HERMES_HOME").ok();

        let test_path = PathBuf::from("/tmp/test_override");
        let token = set_operant_home_override(&test_path);
        assert_eq!(get_operant_home(), test_path);
