...
//!
//! This mirrors the upstream Operant context-file behavior at a smaller scope:
...
/// Load all `.md` and `.txt` files from a Operant context directory.
...
/// Load context files from `~/.config/operant/context` when available.
pub fn load_default_context_files() -> String {
    dirs::config_dir()
        .map(|dir| dir.join("operant").join("context"))
