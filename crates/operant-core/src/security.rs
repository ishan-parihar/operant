...
/// 2. `PATH` lookup.
/// 3. Operant data directory (`<operant_data>/bin/tirith`).
...
async fn resolve_tirith_path(config: &TirithConfig) -> Result<PathBuf> {
    ...
    // 3. Operant data directory
    let data_bin = platform::operant_data_dir().join("bin").join("tirith");
...
async fn auto_install_tirith() -> Result<PathBuf> {
    ...
    let dest_dir = platform::operant_data_dir().join("bin");
