...
/// initialization cost.
pub async fn register_lifeos_tools(registry: &ToolRegistry, state: LifeosState) -> Result<()> {
    registry
        .register(LifeosQueryTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosMutateTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosGetSchemaTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosIntelligenceTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosReviewTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosStrategicTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosSyncNoteTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosHolonicSynthesisTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosEnergyFlowTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosGetPageTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosBuildContextTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosTraceTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosAncestorsTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosBacklinksTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosLinkTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosUnlinkTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosBatchLinkTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosOrphansTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosValidateTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosSuggestLinksTool {
            state: state.clone(),
        })
        .await?;
    registry
        .register(LifeosDailyTool {
            state: state.clone(),
        })
        .await?;
...
fn run_lifeos_tool(tool_name: &str, result: std::result::Result<String, String>) -> ToolResult {
    match result {
...
pub struct LifeosGetSchemaTool {
    state: LifeosState,
}
...
impl OperantTool for LifeosGetSchemaTool {
    ...
    async fn execute(&self, args: Value, _ctx: ToolContext) -> ToolResult {
        ...
        ToolResult::success(
            "lifeos_get_schema",
            serde_json::json!({"database": db, "properties": props}),
        )
    }
}
...
pub struct LifeosIntelligenceTool {
    state: LifeosState,
}
...
impl OperantTool for LifeosIntelligenceTool {
    ...
    fn description(&self) -> &str {
        "Generate an intelligence briefing from LifeOS data."
    }
    fn schema(&self) -> ToolSchema {
        schema_from_json(
            "lifeos_intelligence",
...
impl OperantTool for LifeosReviewTool {
    ...
    fn description(&self) -> &str {
        "Run a review pipeline over LifeOS entries."
    }
    fn schema(&self) -> ToolSchema {
        schema_from_json(
            "lifeos_review",
    ...
    async fn execute(&self, args: Value, _ctx: ToolContext) -> ToolResult {
        ...
        let result =
            lifeos_core::tools::review::execute(&params, &self.state.config, &self.state.notion)
                .await;
        run_lifeos_tool("lifeos_review", result)
    }
}

pub struct LifeosStrategicTool {
    state: LifeosState,
}
...
impl OperantTool for LifeosStrategicTool {
    ...
    fn description(&self) -> &str {
        "Run a strategic simulation over LifeOS data."
    }
    fn schema(&self) -> ToolSchema {
        schema_from_json(
            "lifeos_strategic",
    ...
    async fn execute(&self, args: Value, _ctx: ToolContext) -> ToolResult {
        ...
        let result =
            lifeos_core::tools::strategic::execute(&params, &self.state.config, &self.state.notion)
                .await;
        run_lifeos_tool("lifeos_strategic", result)
    }
}

pub struct LifeosSyncNoteTool {
    state: LifeosState,
}
...
impl OperantTool for LifeosSyncNoteTool {
    ...
    fn description(&self) -> &str {
        "Sync a note to LifeOS Notion databases."
    }
    fn schema(&self) -> ToolSchema {
        schema_from_json(
            "lifeos_sync_note",
    ...
    async fn execute(&self, args: Value, _ctx: ToolContext) -> ToolResult {
        let params: lifeos_core::tools::sync_note::SyncNoteParams =
            match serde_json::from_value(args) {
                Ok(p) => p,
                Err(e) => return ToolResult::error("lifeos_sync_note", format!("args: {e}")),
            };
        let result =
            lifeos_core::tools::sync_note::execute(&params, &self.state.config, &self.state.notion)
                .await;
...
pub struct LifeosHolonicSynthesisTool {
    state: LifeosState,
}
...
impl OperantTool for LifeosHolonicSynthesisTool {
    fn name(&self) -> &str {
        "lifeos_holonic_synthesis"
    }
    fn description(&self) -> &str {
        "Run holonic synthesis over LifeOS entries."
    }
    fn schema(&self) -> ToolSchema {
        schema_from_json(
            "lifeos_holonic_synthesis",
    ...
    async fn execute(&self, args: Value, _ctx: ToolContext) -> ToolResult {
        let params: lifeos_core::tools::holonic_synthesis::HolonicSynthesisParams =
            match serde_json::from_value(args) {
                Ok(p) => p,
                Err(e) => {
                    return ToolResult::error("lifeos_holonic_synthesis", format!("args: {e}"))
                }
            };
...
pub struct LifeosEnergyFlowTool {
    state: LifeosState,
}
...
impl OperantTool for LifeosEnergyFlowTool {
    ...
    fn description(&self) -> &str {
        "Analyze energy flow across LifeOS entries."
    }
    fn schema(&self) -> ToolSchema {
        schema_from_json(
            "lifeos_energy_flow",
...
lifeos_relational_tool!(
    LifeosGetPageTool,
    "lifeos_get_page",
    lifeos_core::tools::relations::execute_get_page,
    schema_get_page,
    lifeos_core::tools::relations::GetPageParams,
    "Get a single LifeOS page with all properties and relations."
);
lifeos_relational_tool!(
    LifeosTraceTool,
    "lifeos_trace",
    lifeos_core::tools::relations::execute_trace,
    schema_trace,
    lifeos_core::tools::relations::TraceParams,
    "Trace relational paths from a starting entry."
);
lifeos_relational_tool!(
    LifeosAncestorsTool,
    "lifeos_ancestors",
    lifeos_core::tools::relations::execute_ancestors,
    schema_ancestors,
    lifeos_core::tools::relations::AncestorsParams,
    "Get all ancestor entries of a LifeOS entry."
);
lifeos_relational_tool!(
    LifeosBacklinksTool,
    "lifeos_backlinks",
    lifeos_core::tools::relations::execute_backlinks,
    schema_backlinks,
    lifeos_core::tools::relations::BacklinksParams,
    "Get all entries that link to a LifeOS entry."
);

pub struct LifeosBuildContextTool {
    state: LifeosState,
}
...
impl OperantTool for LifeosBuildContextTool {
    fn name(&self) -> &str {
        "lifeos_build_context"
    }
    fn description(&self) -> &str {
        "Build a context window from LifeOS entries for LLM consumption."
    }
    fn schema(&self) -> ToolSchema {
        schema_from_json(
            "lifeos_build_context",
...
lifeos_relational_tool!(
    LifeosLinkTool,
    "lifeos_link",
    lifeos_core::tools::relations::execute_link,
    schema_link,
    lifeos_core::tools::relations::LinkParams,
    "Create a relation between two LifeOS entries."
);
lifeos_relational_tool!(
    LifeosUnlinkTool,
    "lifeos_unlink",
    lifeos_core::tools::relation_ops::execute_unlink,
    schema_link,
    lifeos_core::tools::relation_ops::UnlinkParams,
    "Remove a relation between two LifeOS entries."
);
lifeos_relational_tool!(
    LifeosBatchLinkTool,
    "lifeos_batch_link",
    lifeos_core::tools::relation_ops::execute_batch_link,
    schema_link,
    lifeos_core::tools::relation_ops::BatchLinkParams,
...
macro_rules! lifeos_audit_tool {
    ($tool_name:ident, $tool_str:expr, $exec_fn:ident, $schema_fn:ident, $params_ty:ty, $desc:expr) => {
        ...
        impl OperantTool for $tool_name {
            ...
            fn schema(&self) -> ToolSchema {
                schema_from_json($tool_str, $desc, lifeos_core::tools::audit::$schema_fn())
            }
            async fn execute(&self, args: Value, _ctx: ToolContext) -> ToolResult {
                let params: $params_ty = match serde_json::from_value(args) {
                    Ok(p) => p,
...
lifeos_audit_tool!(
    ...
    execute_orphans,
    schema_orphans,
    lifeos_core::tools::audit::OrphansParams,
    "Find orphan entries with no relations."
);
lifeos_audit_tool!(
    LifeosValidateTool,
    "lifeos_validate",
    execute_validate,
    schema_validate,
    lifeos_core::tools::audit::ValidateParams,
    "Validate LifeOS entries for schema compliance."
);
lifeos_audit_tool!(
    LifeosSuggestLinksTool,
    "lifeos_suggest_links",
    execute_suggest_links,
    schema_suggest_links,
    lifeos_core::tools::audit::SuggestLinksParams,
    "Suggest potential links between entries."
);
...
impl OperantTool for LifeosDailyTool {
    ...
    fn description(&self) -> &str {
        "Generate a daily briefing from LifeOS data."
    }
    fn schema(&self) -> ToolSchema {
        schema_from_json(
            "lifeos_daily",
...
pub struct LifeosDashboardTool {
    state: LifeosState,
}
...
impl OperantTool for LifeosDashboardTool {
    ...
    fn description(&self) -> &str {
        "Generate a dashboard view of LifeOS status."
    }
    fn schema(&self) -> ToolSchema {
        schema_from_json(
            "lifeos_dashboard",
...
mod tests {
    ...
    fn lifeos_tool_names_are_prefixed() {
        ...
        for name in &names {
            assert!(
                name.starts_with("lifeos_"),
                "tool '{}' should start with 'lifeos_'",
                name
            );
        }
    }
