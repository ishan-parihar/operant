...
#[async_trait]
impl OperantTool for RlTrainingTool {
    fn name(&self) -> &str {
        "rl"
    }
...
impl RlTrainingTool {
    fn scan_framework_envs() -> Vec<EnvironmentInfo> {
        let operant_home = std::env::var("HERMES_HOME").unwrap_or_else(|_| {
            dirs::home_dir()
                .map(|p| p.join(".operant").to_string_lossy().to_string())
                .unwrap_or_else(|| "~/.operant".to_string())
        });
        let envs_dir = std::path::Path::new(&operant_home).join("environments");
    ...
    async fn handle_start_training(&self, args: &RlArgs) -> ToolResult {
        ...
        let mut cmd = Command::new("operant");
        cmd.args([
            "environment",
            "launch_training",
            "--environment",
