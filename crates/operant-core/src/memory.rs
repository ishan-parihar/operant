//! Memory and session management for Operant-RS
//!
...
/// using the `§` (section sign) delimiter between entries to match the Python operant-agent format.
#[derive(Debug, Clone)]
...
/// the `[memory]` section of `operant.toml`.
///
...
impl HindsightMemoryClient {
    ...
    const DEFAULT_BANK_ID: &'static str = "operant";
