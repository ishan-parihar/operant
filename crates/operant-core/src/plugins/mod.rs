...
//! # Discovery
//!
//! [`discover_plugins`] scans directories (e.g. `~/.operant/plugins/`) for
...
mod tests {
    ...
    fn test_discover_plugins_skips_non_existent() {
        let dir = PathBuf::from("/tmp/operant_plugin_nonexistent_should_not_exist_42");
    ...
    fn test_discover_plugins_toml() {
        let dir =
            std::env::temp_dir().join(format!("operant_plugin_toml_test_{}", std::process::id()));
...
"#,
    ...
    fn test_discover_plugins_yaml() {
        let dir =
            std::env::temp_dir().join(format!("operant_plugin_yaml_test_{}", std::process::id()));
