...
//!
//! Ported from `operant-agent/tools/skills_hub.py` (~3,261 LOC Python).
...
/// Default Operant home subdirectory for skills
...
/// Operant centralized index URL
const HERMES_INDEX_URL: &str = "https://operant-agent.nousresearch.com/docs/api/skills-index.json";
/// Operant index TTL: 6 hours
...
/// Resolve the skills hub paths relative to a base directory (e.g. `~/.operant`).
#[derive(Debug, Clone)]
pub struct HubPaths {
...
impl HubPaths {
    /// Create a new `HubPaths` rooted at `operant_home/skills/`.
    pub fn new(operant_home: &Path) -> Self {
        let skills_dir = operant_home.join(SKILLS_DIR_NAME);
...
impl GitHubSource {
    ...
    fn fm_tags(fm: &HashMap<String, serde_json::Value>) -> Vec<String> {
        // Try metadata.operant.tags
        if let Some(meta) = fm.get("metadata").and_then(|v| v.as_object()) {
            if let Some(operant) = meta.get("operant").and_then(|v| v.as_object()) {
                if let Some(tags) = operant.get("tags").and_then(|v| v.as_array()) {
...
// ---------------------------------------------------------------------------
// Operant centralized index source
// ---------------------------------------------------------------------------

/// Skill source backed by the centralized Operant Skills Index.
pub struct OperantIndexSource {
    auth: GitHubAuth,
    paths: HubPaths,
    client: reqwest::Client,
...
impl OperantIndexSource {
    ...
    async fn ensure_loaded(&self) -> serde_json::Value {
        ...
        let index = self.load_operant_index().await;
        *self.index.write().await = index.clone();
        *self.loaded.write().await = true;
        index.unwrap_or(serde_json::Value::Null)
    }

    async fn load_operant_index(&self) -> Option<serde_json::Value> {
        let cache_file = self.paths.index_cache_dir.join("operant-index.json");
...
#[async_trait]
impl SkillSource for OperantIndexSource {
    fn source_id(&self) -> &'static str {
        "operant-index"
    }
...
pub fn create_source_router(
    ...
    // Operant centralized index
    sources.push(Arc::new(OperantIndexSource::new(
        auth.clone(),
        paths.clone(),
    )));
...
pub async fn parallel_search_sources(
    ...
    // Check if the Operant index is available (skip external APIs if so)
    let mut index_available = false;
    if source_filter == "all" {
        for src in sources {
            if src.source_id() == "operant-index" {
...
mod tests {
    ...
    #[test]
    fn test_hub_paths() {
        let base = PathBuf::from("/tmp/test-operant");
    ...
    fn test_hub_lock_file_roundtrip() {
        let tmp = std::env::temp_dir().join("operant_lock_test");
    ...
    fn test_taps_manager() {
        let tmp = std::env::temp_dir().join("operant_taps_test");
        let _ = std::fs::remove_dir_all(&tmp);
        let paths = HubPaths::new(&tmp);
    ...
    fn test_quarantine_and_install_roundtrip() {
        let tmp = std::env::temp_dir().join("operant_quarantine_test");
        let _ = std::fs::remove_dir_all(&tmp);
        let paths = HubPaths::new(&tmp);
    ...
    fn test_uninstall_nonexistent() {
        let tmp = std::env::temp_dir().join("operant_uninstall_nonexistent");
        let _ = std::fs::remove_dir_all(&tmp);
        let paths = HubPaths::new(&tmp);
