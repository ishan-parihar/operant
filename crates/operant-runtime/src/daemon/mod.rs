...
use operant_memory::{MEMORY_CONTEXT_CLOSE, MEMORY_CONTEXT_OPEN};
...
pub struct DaemonSubsystems {
    ...
    pub mqtt_start: Option<
        Box<
            dyn Fn(
                    operant_config::schema::MqttConfig,
                ) -> std::pin::Pin<Box<dyn Future<Output = Result<()>> + Send>>
                + Send
                + Sync,
        >,
...
async fn run_heartbeat_worker(config: Config) -> Result<()> {
    ...
    loop {
        ...
        let heartbeat_memory: Option<Box<dyn operant_memory::Memory>> =
            operant_memory::create_memory(
                &config.memory,
                    ...
                    Ok(entries) if !entries.is_empty() => {
                        let ctx: String = entries
                            ...
                            .filter(|e| {
                                !matches!(
                                    e.category,
                                    operant_memory::traits::MemoryCategory::Conversation
                                )
                            })
                        ...
                        && let Some(ref mem) = heartbeat_memory
                    {
                        ...
                        let _ = mem
                            .store(
                                ...
                                operant_memory::MemoryCategory::Daily,
                                None,
                            )
                            .await;
                    }
...
fn load_heartbeat_session_context(config: &Config) -> Option<String> {
    use operant_providers::traits::ChatMessage;
...
fn load_jsonl_messages(path: &std::path::Path) -> Vec<operant_providers::traits::ChatMessage> {
    ...
    let mut window: VecDeque<operant_providers::traits::ChatMessage> =
    ...
    for line in reader.lines() {
        ...
        if let Ok(msg) = serde_json::from_str::<operant_providers::traits::ChatMessage>(trimmed) {
...
mod tests {
    ...
    fn detects_supervised_channels_present() {
        ...
        config.channels.telegram = Some(operant_config::schema::TelegramConfig {
            ...
            allowed_users: vec![],
            stream_mode: operant_config::schema::StreamMode::default(),
    ...
    fn detects_dingtalk_as_supervised_channel() {
        let mut config = Config::default();
        config.channels.dingtalk = Some(operant_config::schema::DingTalkConfig {
            enabled: true,
    ...
    fn detects_qq_as_supervised_channel() {
        let mut config = Config::default();
        config.channels.qq = Some(operant_config::schema::QQConfig {
            enabled: true,
            app_id: "app-id".into(),
    ...
    fn detects_nextcloud_talk_as_supervised_channel() {
        let mut config = Config::default();
        config.channels.nextcloud_talk = Some(operant_config::schema::NextcloudTalkConfig {
            enabled: true,
    ...
    fn webhook_only_config_is_supervised() {
        let mut config = Config::default();
        config.channels.webhook = Some(operant_config::schema::WebhookConfig {
            enabled: true,
            port: 8080,
            listen_path: None,
            send_url: None,
