...
use operant_memory::traits::Memory;
use operant_providers::multimodal;

pub use operant_config::scattered_types::ContextCompressionConfig;
        ...
        if let Some(ref memory) = self.memory {
            ...
            if let Err(e) = memory
                .store(
                    &facts_key,
                    &summary,
                    operant_memory::traits::MemoryCategory::Daily,
                    None,
                )
                .await
            {
...
mod tests {
    ...
    impl Provider for CaptureSummarizerProvider {
        ...
        async fn chat(
            ...
            _model: &str,
            _temperature: Option<f64>,
        ) -> Result<operant_api::provider::ChatResponse> {
