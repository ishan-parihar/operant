...
use operant_providers::router::{Route, RouterProvider};
use tempfile::TempDir;
...
impl Provider for VisionProvider {
    ...
    async fn chat(
        ...
        self.calls.fetch_add(1, Ordering::SeqCst);
        let marker_count = operant_providers::multimodal::count_image_markers(request.messages);
        if marker_count == 0 {
...
impl Provider for StreamingScriptedProvider {
    ...
    fn stream_chat_with_history(
        ...
        'static,
        operant_providers::traits::StreamResult<StreamChunk>,
    > {
...
async fn run_tool_call_loop_skips_oversized_image_payload() {
    ...
    let tools_registry: Vec<Box<dyn Tool>> = Vec::new();
    let observer = NoopObserver;
    let multimodal = operant_config::schema::MultimodalConfig {
        max_images: 4,
        max_image_size_mb: 1,
...
{"name":"shell","arguments":{"command":"echo hello"}}
    ...
    let observer = NoopObserver;
    let approval_mgr =
        ApprovalManager::for_non_interactive(&operant_config::schema::AutonomyConfig::default());
...
async fn cost_tracking_records_usage_when_scoped() {
    ...
    use operant_config::schema::ModelPricing;
    ...
    let provider = ScriptedProvider {
        responses: Arc::new(Mutex::new(VecDeque::from([ChatResponse {
            text: Some("done".to_string()),
            tool_calls: Vec::new(),
            usage: Some(operant_providers::traits::TokenUsage {
    ...
    let mut cost_config = operant_config::schema::CostConfig {
        enabled: true,
        ..operant_config::schema::CostConfig::default()
    };
...
async fn cost_tracking_enforces_budget() {
    ...
    let observer = NoopObserver;
    let workspace = tempfile::TempDir::new().unwrap();
    let cost_config = operant_config::schema::CostConfig {
        enabled: true,
