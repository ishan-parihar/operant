...
pub struct Agent {
    ...
    config: operant_config::schema::AgentConfig,
    multimodal_config: operant_config::schema::MultimodalConfig,
    model_name: String,
    provider_name: String,
    temperature: f64,
    workspace_dir: std::path::PathBuf,
    identity_config: operant_config::schema::IdentityConfig,
    skills: Vec<crate::skills::Skill>,
    skills_prompt_mode: operant_config::schema::SkillsPromptInjectionMode,
    auto_save: bool,
...
impl AgentChannelHandles {
    ...
    pub fn register_channel(
        ...
        channel: Arc<dyn operant_api::channel::Channel>,
    ) {
        let name = name.into();
    ...
    pub fn get_channel(&self, name: &str) -> Option<Arc<dyn operant_api::channel::Channel>> {
        for handle in [&self.ask_user, &self.reaction, &self.escalate, &self.poll]
            .into_iter()
            .flatten()
        {
...
pub struct AgentBuilder {
    ...
    config: Option<operant_config::schema::AgentConfig>,
    multimodal_config: Option<operant_config::schema::MultimodalConfig>,
    model_name: Option<String>,
    provider_name: Option<String>,
    temperature: Option<f64>,
    workspace_dir: Option<std::path::PathBuf>,
    identity_config: Option<operant_config::schema::IdentityConfig>,
    skills: Option<Vec<crate::skills::Skill>>,
    skills_prompt_mode: Option<operant_config::schema::SkillsPromptInjectionMode>,
    auto_save: Option<bool>,
    memory_session_id: Option<String>,
    classification_config: Option<operant_config::schema::QueryClassificationConfig>,
...
impl AgentBuilder {
    ...
    pub fn config(mut self, config: operant_config::schema::AgentConfig) -> Self {
    ...
    pub fn response_cache(
        mut self,
        cache: Option<Arc<operant_memory::response_cache::ResponseCache>>,
    ) -> Self {
...
impl Agent {
    ...
    fn parse_response_for_effective_tools(
        &self,
        response: &operant_providers::ChatResponse,
    ...
    async fn from_config_with_session_cwd_and_mcp_approval_mode(
        ...
        let response_cache = if config.memory.response_cache_enabled {
            operant_memory::response_cache::ResponseCache::with_hot_cache(
                &config.workspace_dir,
    ...
    async fn prepare_provider_messages(
        ...
        let prepared = operant_providers::multimodal::prepare_messages_for_provider(
            messages,
    ...
    async fn execute_tool_call(&self, call: &ParsedToolCall) -> ToolExecutionResult {
            ...
            && approval_requirement == ApprovalRequirement::Prompt
        {
            ...
            let (decision, decision_channel) = if mgr.is_non_interactive() {
                ...
                let mut channel_decision: Option<operant_api::channel::ChannelApprovalResponse> =
                    None;
                ...
                let channels: Vec<(String, Arc<dyn operant_api::channel::Channel>)> = self
                    .channel_handles
                    .ask_user
                    .as_ref()
                    .map(|h| {
            ...
            let cache_key = if self.temperature == 0.0 {
                self.response_cache.as_ref().map(|_| {
                    ...
                    let system = messages
                        ...
                        .map(|m| m.content.as_str());
                    operant_memory::response_cache::ResponseCache::cache_key(
                        &effective_model,
                        system,
                        last_user,
                    )
    ...
    pub async fn turn_streamed(
        ...
        for _ in 0..self.config.max_tool_iterations {
            ...
            let stream_opts =
                operant_providers::traits::StreamOptions::new(self.provider.supports_streaming());
            let mut stream = self.provider.stream_chat(
                operant_providers::ChatRequest {
                    messages: &prepared_messages,
                    tools: if self.should_send_tool_specs() {
                        Some(&self.tool_specs)
                    } else {
            ...
            let mut streamed_tool_calls: Vec<operant_providers::traits::ToolCall> = Vec::new();
            let mut streamed_usage: Option<operant_providers::traits::TokenUsage> = None;
            ...
            loop {
                ...
                match item {
                    Ok(event) => match event {
                        operant_providers::traits::StreamEvent::TextDelta(chunk) => {
                        ...
                        operant_providers::traits::StreamEvent::PreExecutedToolResult {
                            name,
                            output,
                        } => {
                        ...
                        operant_providers::traits::StreamEvent::Usage(usage) => {
                            streamed_usage = Some(usage);
                        }
                        operant_providers::traits::StreamEvent::Final => break,
                    },
                    Err(_) => break,
                }
            }
            ...
            let response = if got_stream {
                ...
                match chat_result {
                    Ok(resp) => resp,
                    Err(err) => {
                        let safe_error = operant_providers::sanitize_api_error(&err.to_string());
    ...
    pub async fn run_interactive(&mut self) -> Result<()> {
        ...
        let listen_handle = tokio::spawn(async move {
            let _ = operant_api::channel::Channel::listen(&*cli, tx).await;
        });
...
mod tests {
    ...
    struct MockProvider {
        responses: Mutex<Vec<operant_providers::ChatResponse>>,
    }
    ...
    impl Provider for MockProvider {
        ...
        async fn chat(
            ...
            _model: &str,
            _temperature: Option<f64>,
        ) -> Result<operant_providers::ChatResponse> {
            let mut guard = self.responses.lock();
            if guard.is_empty() {
                return Ok(operant_providers::ChatResponse {
                    text: Some("done".into()),
                    tool_calls: vec![],
                    usage: None,
                    reasoning_content: None,
    ...
    impl Provider for MultimodalCaptureProvider {
        ...
        fn stream_chat(
            ...
            _model: &str,
            _temperature: Option<f64>,
            _options: operant_providers::traits::StreamOptions,
        ) -> futures_util::stream::BoxStream<
            'static,
            operant_providers::traits::StreamResult<operant_providers::traits::StreamEvent>,
        > {
            ...
            if self.streamed {
                let chunk = operant_providers::traits::StreamEvent::TextDelta(
                    operant_providers::traits::StreamChunk {
                ...
                stream::iter(vec![
                    Ok(chunk),
                    Ok(operant_providers::traits::StreamEvent::Final),
                ])
                .boxed()
            } else {
                stream::iter(vec![Ok(operant_providers::traits::StreamEvent::Final)]).boxed()
            }
        }
    ...
    struct ApprovalChannel {
        response: operant_api::channel::ChannelApprovalResponse,
        requests: Arc<AtomicUsize>,
    }

    #[async_trait]
    impl operant_api::channel::Channel for ApprovalChannel {
        fn name(&self) -> &str {
            "acp"
        }

        async fn send(&self, _message: &operant_api::channel::SendMessage) -> anyhow::Result<()> {
            Ok(())
        }
        ...
        async fn request_approval(
            &self,
            _recipient: &str,
            _request: &operant_api::channel::ChannelApprovalRequest,
    ...
    fn native_agent_prompt_omits_duplicate_tools_section() {
        ...
        let mem: Arc<dyn Memory> = Arc::from(
            operant_memory::create_memory(&memory_cfg, workspace.path(), None)
                .expect("memory creation should succeed with valid config"),
        );
        let observer: Arc<dyn Observer> = Arc::from(crate::observability::NoopObserver {});
    ...
    async fn direct_agent_tool_execution_requests_acp_approval() {
        ...
        let channel: Arc<dyn operant_api::channel::Channel> = Arc::new(ApprovalChannel {
            response: operant_api::channel::ChannelApprovalResponse::Approve,
    ...
    async fn direct_agent_tool_execution_denies_when_acp_rejects() {
        ...
        agent.channel_handles.ask_user = Some(Arc::clone(&handle));
        let channel: Arc<dyn operant_api::channel::Channel> = Arc::new(ApprovalChannel {
            response: operant_api::channel::ChannelApprovalResponse::Deny,
            requests: Arc::clone(&approval_requests),
        });
        handle.write().insert("acp".to_string(), channel);
    ...
    async fn direct_agent_shell_keeps_runtime_approval_from_always_allowlist() {
        ...
        agent.channel_handles.ask_user = Some(Arc::clone(&handle));
        let channel: Arc<dyn operant_api::channel::Channel> = Arc::new(ApprovalChannel {
            response: operant_api::channel::ChannelApprovalResponse::AlwaysApprove,
            requests: Arc::clone(&approval_requests),
        });
        handle.write().insert("acp".to_string(), channel);
        ...
        let provider = Box::new(MockProvider {
            responses: Mutex::new(vec![
                operant_providers::ChatResponse {
                    ...
                    tool_calls: vec![operant_providers::ToolCall {
                        id: "tc1".into(),
                        name: "echo".into(),
                        arguments: "{}".into(),
                        extra_content: None,
    ...
    fn session_cwd_keeps_workspace_in_allowed_roots() {
        ...
        let autonomy = operant_config::schema::AutonomyConfig::default();
    ...
    impl Provider for StreamToolCaptureProvider {
        ...
        > {
            ...
            *count += 1;
            if *count == 1 {
                let tc =
                    operant_providers::traits::StreamEvent::ToolCall(operant_providers::ToolCall {
                        id: "00000000-0000-0000-0000-000000000001".into(),
                        name: "echo".into(),
                        arguments: "{}".into(),
                        extra_content: None,
    ...
    fn trim_history_does_not_leave_orphan_tool_results() {
        use operant_providers::{ToolCall, ToolResultMessage};
        ...
        let agent_config = operant_config::schema::AgentConfig {
    ...
    impl Provider for NarrationStreamProvider {
        ...
        > {
            ...
            *count += 1;
            if *count == 1 {
                stream::iter(vec![
                    Ok(operant_providers::traits::StreamEvent::TextDelta(
                        operant_providers::traits::StreamChunk {
                    ...
                    Ok(operant_providers::traits::StreamEvent::ToolCall(
                        operant_providers::ToolCall {
                            id: "tc1".into(),
                            name: "echo".into(),
                            arguments: "{}".into(),
                            extra_content: None,
