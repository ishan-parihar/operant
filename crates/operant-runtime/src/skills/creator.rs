...
// under `~/.operant/workspace/skills/<slug>/`.

use anyhow::{Context, Result};
use operant_config::schema::SkillCreationConfig;
use operant_memory::embeddings::EmbeddingProvider;
use operant_memory::vector::cosine_similarity;
...
impl SkillCreator {
    ...
    fn generate_skill_toml(slug: &str, description: &str, tool_calls: &[ToolCallRecord]) -> String {
        ...
        toml.push_str("author = \"operant-auto\"\n");
    ...
    async fn enforce_lru_limit(&self) -> Result<()> {
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            let content = tokio::fs::read_to_string(&toml_path).await?;
            if content.contains("\"operant-auto\"") || content.contains("\"auto-generated\"") {
...
mod tests {
    ...
    use operant_memory::embeddings::{EmbeddingProvider, NoopEmbedding};
...
"#
    ...
    async fn create_from_execution_success() {
        ...
        assert!(toml_content.contains("operant-auto"));
    }
