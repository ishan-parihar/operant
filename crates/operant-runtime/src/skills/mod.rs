...
pub fn skills_to_prompt_with_mode(
    skills: &[Skill],
    workspace_dir: &Path,
    mode: operant_config::schema::SkillsPromptInjectionMode,
) -> String {
    use std::fmt::Write;
    ...
    let mut prompt = match mode {
        operant_config::schema::SkillsPromptInjectionMode::Full => String::from(
            "## Available Skills\n\n\
             Skill instructions and tool metadata are preloaded below.\n\
             Follow these instructions directly; do not read skill files at runtime unless the user asks.\n\n\
             <available_skills>\n",
        ),
        operant_config::schema::SkillsPromptInjectionMode::Compact => String::from(
            "## Available Skills\n\n\
...
) -> Vec<Box<dyn operant_api::tool::Tool>> {
    let mut tools: Vec<Box<dyn operant_api::tool::Tool>> = Vec::new();
    for skill in skills {
        for tool in &skill.tools {
            match tool.kind.as_str() {
                "shell" | "script" => {
                    ...
                    tools.push(Box::new(operant_tools::wrappers::RateLimitedTool::new(
                        inner,
                        security.clone(),
                    )));
                }
...
#[cfg(feature = "plugins-wasm")]
pub fn load_plugin_skills_from_config(config: &operant_config::schema::Config) -> Vec<Skill> {
    ...
    let signature_mode = operant_plugins::host::PluginHost::parse_signature_mode(
