...
//! - AI providers: `operant_providers::list_providers()` (each
...
use operant_providers::ProviderActivation;
...
fn evaluate_provider_activation(
    config: &Config,
    info: &operant_providers::ProviderInfo,
) -> IntegrationStatus {
...
pub fn all_integrations(config: &Config) -> Vec<IntegrationEntry> {
    ...
    let providers = operant_providers::list_providers().into_iter().map(|info| {
...
mod tests {
    use super::*;
    use operant_config::schema::Config;
    use operant_config::schema::{IMessageConfig, MatrixConfig, StreamMode, TelegramConfig};
    ...
    fn regional_provider_aliases_activate_expected_ai_integrations() {
        ...
        for (fallback_key, canonical) in cases {
            ...
            let entries = all_integrations(&config);
            let info = operant_providers::list_providers()
                .into_iter()
