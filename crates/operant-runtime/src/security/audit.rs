...
use operant_config::schema::AuditConfig;
...
pub struct AuditLogger {
    log_path: PathBuf,
    config: AuditConfig,
    #[expect(dead_code, reason = "WIP: buffered writes for batch flushing")]
    // WIP: buffered writes for batch flushing
...
impl AuditLogger {
    ...
    ///
    /// If `config.sign_events` is true, requires `OPERANT_AUDIT_SIGNING_KEY` env var
    /// to be set with a hex-encoded 32-byte key. Fails if key is missing or invalid.
    pub fn new(config: AuditConfig, operant_dir: PathBuf) -> Result<Self> {
        // Load and validate signing key if sign_events enabled
        let signing_key = if config.sign_events {
            let key_hex = std::env::var("OPERANT_AUDIT_SIGNING_KEY").map_err(|_| {
                anyhow::anyhow!("sign_events enabled but OPERANT_AUDIT_SIGNING_KEY not set")
            })?;

            let key_bytes = hex::decode(&key_hex)
                .map_err(|_| anyhow::anyhow!("OPERANT_AUDIT_SIGNING_KEY must be hex-encoded"))?;

            if key_bytes.len() != 32 {
                bail!(
                    "OPERANT_AUDIT_SIGNING_KEY must be 32 bytes (64 hex chars), got {}",
                    key_bytes.len()
                );
            }
        ...
        let log_path = operant_dir.join(&config.log_path);
...
/// - If a record has a `signature` field and `OPERANT_AUDIT_SIGNING_KEY` is available,
...
pub fn verify_chain(log_path: &Path) -> Result<u64> {
    ...
    let signing_key = std::env::var("OPERANT_AUDIT_SIGNING_KEY")
        .ok()
...
mod tests {
    ...
    /// Mutex to serialize tests that read/write OPERANT_AUDIT_SIGNING_KEY env var.
    ...
    fn audit_event_with_actor() {
        ...
        assert_eq!(actor.username, Some("@operant_user".to_string()));
    }
    ...
    fn signature_present_when_sign_events_enabled() -> Result<()> {
        ...
        let old_key = std::env::var("OPERANT_AUDIT_SIGNING_KEY").ok();
        defer! {
            if let Some(key) = old_key {
                // SAFETY: test-only, single-threaded test runner.
                unsafe { std::env::set_var("OPERANT_AUDIT_SIGNING_KEY", key) };
            } else {
                // SAFETY: test-only, single-threaded test runner.
                unsafe { std::env::remove_var("OPERANT_AUDIT_SIGNING_KEY") };
            }
        }
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("OPERANT_AUDIT_SIGNING_KEY", &test_key) };
    ...
    fn constructor_fails_if_sign_events_but_no_key() -> Result<()> {
        ...
        if let Err(e) = result {
            ...
            assert!(
                err_msg.contains("OPERANT_AUDIT_SIGNING_KEY not set"),
                "error: {}",
                err_msg
            );
        }
    ...
    fn constructor_fails_if_signing_key_invalid_hex() -> Result<()> {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("OPERANT_AUDIT_SIGNING_KEY", "not-valid-hex") };
    ...
    fn constructor_fails_if_signing_key_wrong_length() -> Result<()> {
        ...
        // SAFETY: test-only, single-threaded test runner.
        unsafe { std::env::set_var("OPERANT_AUDIT_SIGNING_KEY", &short_key) };
        let tmp = TempDir::new()?;
        let config = AuditConfig {
            enabled: true,
            sign_events: true,
