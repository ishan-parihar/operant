...
mod tests {
    ...
    use operant_tools::wrappers::{PathGuardedTool, RateLimitedTool};
    ...
    async fn shell_does_not_leak_api_key() {
        ...
        let _g2 = EnvGuard::set("OPERANT_API_KEY", "sk-test-secret-67890");
        ...
        assert!(
            ...
            "OPERANT_API_KEY leaked to shell command output"
        );
    }
    ...
    async fn shell_allows_configured_env_passthrough() {
        let _guard = EnvGuard::set("OPERANT_TEST_PASSTHROUGH", "db://unit-test");
        let tool = ShellTool::new(
            test_security_with_env_passthrough(&["OPERANT_TEST_PASSTHROUGH"]),
            test_runtime(),
        );
        ...
        assert!(
            result
                .output
                .contains("OPERANT_TEST_PASSTHROUGH=db://unit-test")
        );
    }
    ...
    async fn shell_requires_approval_for_medium_risk_command() {
        ...
        let denied = tool
            .execute(json!({"command": "touch operant_shell_approval_test"}))
            .await
        ...
        let allowed = tool
            .execute(json!({
                "command": "touch operant_shell_approval_test",
                "approved": true
            }))
            .await
        ...
        let _ =
            tokio::fs::remove_file(std::env::temp_dir().join("operant_shell_approval_test")).await;
    }
