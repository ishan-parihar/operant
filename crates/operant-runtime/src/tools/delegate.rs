...
impl DelegateTool {
    ...
    pub fn with_multimodal_config(
        mut self,
        config: operant_config::schema::MultimodalConfig,
    ) -> Self {
    ...
    #[expect(
        dead_code,
        reason = "WIP: will be used when delegate agents support memory"
    )] // WIP: will be used when delegate agents support memory
...
impl DelegateTool {
    ...
    async fn execute_sync(
        ...
        let provider: Box<dyn Provider> = match operant_providers::create_provider_with_options(
...
mod tests {
    ...
    use operant_providers::{ChatRequest, ChatResponse, ToolCall};
    use std::path::Path;
    ...
    fn enriched_prompt_falls_back_to_default_skills_dir() {
        let workspace = std::env::temp_dir().join(format!(
            "operant_delegate_fallback_test_{}",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn background_delegation_returns_task_id() {
        let workspace =
            std::env::temp_dir().join(format!("operant_delegate_bg_test_{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn check_result_retrieves_persisted_background_result() {
        let workspace = std::env::temp_dir().join(format!(
            "operant_delegate_check_retrieve_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn check_result_rejects_path_traversal() {
        let workspace = std::env::temp_dir().join(format!(
            "operant_delegate_traversal_check_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    async fn cancel_task_rejects_path_traversal() {
        let workspace = std::env::temp_dir().join(format!(
            "operant_delegate_traversal_cancel_{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&workspace).unwrap();
