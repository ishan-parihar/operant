...
use operant_config::schema::{CronJobDecl, CronScheduleDecl};
...
async fn run_agent_job(
    ...
    let memory_context = if !job.uses_memory {
        ...
        match operant_memory::create_memory(
            &config.memory,
            &config.workspace_dir,
            config
                .providers
    ...
    match run_result {
        ...
        Err(e) => {
            ...
            let mem_session_key = operant_api::session_keys::sanitize_session_key(&format!(
                "cli:{}",
                session_path.display()
            ));
            if let Ok(mem) = operant_memory::create_memory(
                &config.memory,
                &config.workspace_dir,
                config
                    .providers
