...
pub fn sync_declarative_jobs(
    config: &Config,
    decls: &[operant_config::schema::CronJobDecl],
) -> Result<()> {
    use operant_config::schema::CronScheduleDecl;
...
fn validate_decl(decl: &operant_config::schema::CronJobDecl) -> Result<()> {
...
fn convert_schedule_decl(decl: &operant_config::schema::CronScheduleDecl) -> Result<Schedule> {
    use operant_config::schema::CronScheduleDecl;
    match decl {
...
fn convert_delivery_decl(decl: &operant_config::schema::DeliveryConfigDecl) -> DeliveryConfig {
    DeliveryConfig {
...
mod tests {
    ...
    fn make_shell_decl(id: &str, expr: &str, cmd: &str) -> operant_config::schema::CronJobDecl {
        operant_config::schema::CronJobDecl {
            id: id.to_string(),
            name: Some(format!("decl-{id}")),
            job_type: "shell".to_string(),
            schedule: operant_config::schema::CronScheduleDecl::Cron {
                expr: expr.to_string(),
                tz: None,
            },
    ...
    fn make_agent_decl(id: &str, expr: &str, prompt: &str) -> operant_config::schema::CronJobDecl {
        operant_config::schema::CronJobDecl {
            id: id.to_string(),
            name: Some(format!("decl-{id}")),
    ...
    fn sync_every_schedule_works() {
        ...
        let decl = operant_config::schema::CronJobDecl {
            id: "interval-job".to_string(),
            name: None,
            job_type: "shell".to_string(),
            schedule: operant_config::schema::CronScheduleDecl::Every { every_ms: 60000 },
...
schedule = { kind = "every", every_ms = 300000 }
        ...
        let parsed: operant_config::schema::CronConfig = toml::from_str(toml_str).unwrap();
        ...
        assert!(matches!(
            ...
            operant_config::schema::CronScheduleDecl::Cron { ref expr, .. } if expr == "0 9 * * *"
        ));
        ...
        assert!(matches!(
            ...
            operant_config::schema::CronScheduleDecl::Every { every_ms: 300_000 }
        ));
    }
}
