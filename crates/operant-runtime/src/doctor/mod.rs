...
async fn probe_models(config: &Config) -> Vec<DiagResult> {
    ...
    for provider_name in &targets {
        let result = match operant_providers::create_provider(provider_name, None) {
...
pub async fn run_models(
    ...
    for provider_name in &targets {
        ...
        let outcome = match operant_providers::create_provider(provider_name, None) {
            Ok(handle) => handle.list_models().await,
            Err(e) => Err(e),
        };
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    for name in agent_names {
        let agent = config
            .agents
            .get(name)
            .expect("agent name validated against config keys");
...
mod tests {
    ...
    fn config_validation_warns_empty_model_route() {
        let mut config = Config::default();
        config.providers.model_routes = vec![operant_config::schema::ModelRouteConfig {
            hint: "fast".into(),
            provider: "groq".into(),
            model: String::new(),
            api_key: None,
    ...
    fn config_validation_warns_empty_embedding_route_model() {
        let mut config = Config::default();
        config.providers.embedding_routes = vec![operant_config::schema::EmbeddingRouteConfig {
            hint: "semantic".into(),
    ...
    fn config_validation_reports_delegate_agents_in_sorted_order() {
        ...
        config.agents.insert(
            "zeta".into(),
            operant_config::schema::DelegateAgentConfig {
