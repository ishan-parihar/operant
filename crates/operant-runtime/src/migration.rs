...
pub async fn migrate_openclaw_memory(
    ...
    if paths_equal(&source_workspace, &config.workspace_dir) {
        bail!("Source workspace matches current Operant workspace; refusing self-migration");
    }
...
fn target_memory_backend(config: &Config) -> Result<Box<dyn Memory>> {
    operant_memory::create_memory_for_migration(&config.memory.backend, &config.workspace_dir)
}
...
mod tests {
    use super::*;
    use operant_config::schema::{Config, MemoryConfig};
    use operant_memory::SqliteMemory;
    use rusqlite::params;
