...
/// - `X-Operant-Timestamp` — unix epoch seconds
/// - `X-Operant-Nonce` — random UUID v4
/// - `X-Operant-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-Operant-Timestamp", timestamp.to_string())
            .header("X-Operant-Nonce", &nonce)
            .header("X-Operant-Signature", &signature)
