...
/// `operant_providers::default_provider_config`, etc.) and copy its
...
trait PropFieldSource {
    fn prop_fields(&self) -> Vec<operant_config::traits::PropFieldInfo>;
}

impl PropFieldSource for operant_config::schema::ModelProviderConfig {
    fn prop_fields(&self) -> Vec<operant_config::traits::PropFieldInfo> {
        operant_config::schema::ModelProviderConfig::prop_fields(self)
    }
}
...
///
/// Source of truth is `operant_providers::default_provider_config(name)`
...
pub fn apply_provider_trait_defaults(cfg: &mut Config, provider: &str, prefix: &str) -> Result<()> {
    ...
    let source_prefix = operant_config::schema::ModelProviderConfig::configurable_prefix();
