...
use operant_config::traits::{Answer, OnboardUi, PropKind, SelectItem};
...
fn is_known_provider_name(provider: &str) -> bool {
    ...
    operant_providers::list_providers().iter().any(|entry| {
...
async fn providers(cfg: &mut Config, ui: &mut dyn OnboardUi, flags: &Flags) -> Result<Nav> {
    ...
    let entries = operant_providers::list_providers();
...
/// schema fields of `operant_providers::default_provider_config(provider)`
...
fn provider_trait_defaults_for_prompts(provider: &str, prefix: &str) -> Vec<FieldDefault> {
    let defaults = operant_providers::default_provider_config(provider);
    let source_dot = format!(
        "{}.",
        operant_config::schema::ModelProviderConfig::configurable_prefix()
    );
    defaults
        .prop_fields()
        .into_iter()
...
async fn prompt_model(cfg: &mut Config, ui: &mut dyn OnboardUi, provider: &str) -> Result<Nav> {
    ...
    let catalog_models = match operant_providers::create_provider(provider, None) {
        Ok(handle) => {
...
async fn memory(cfg: &mut Config, ui: &mut dyn OnboardUi, flags: &Flags) -> Result<Nav> {
    ...
    let backends = operant_memory::selectable_memory_backends();
...
mod tests {
    ...
    use operant_config::schema::{Config, ModelProviderConfig};
    use std::sync::Arc;
