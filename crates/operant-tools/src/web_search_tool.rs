...
impl WebSearchTool {
    ...
    fn resolve_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        if let Some(ref key) = self.boot_brave_api_key
            && !key.is_empty()
            && !operant_config::secrets::SecretStore::is_encrypted(key)
        {
            return Ok(key.clone());
        }
    ...
    fn reload_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        let config: operant_config::schema::Config = toml::from_str(&contents).map_err(|e| {
            anyhow::anyhow!(
...
mod tests {
    ...
    fn test_resolve_brave_api_key_decrypts_encrypted_key() {
        let tmp = tempfile::TempDir::new().unwrap();
        let store = operant_config::secrets::SecretStore::new(tmp.path(), true);
