...
use operant_config::schema::{ClassificationRule, Config, DelegateAgentConfig, ModelRouteConfig};
...
impl ModelRoutingConfigTool {
    ...
    fn load_config_without_env(&self) -> anyhow::Result<Config> {
        ...
        let compat: operant_config::migration::V1Compat =
    ...
    async fn handle_set_default(&self, args: &Value) -> anyhow::Result<ToolResult> {
        ...
        {
            if operant_providers::reliable::is_non_retryable(&probe_err) {
    ...
    async fn probe_model(&self, provider_name: &str, model: &str) -> anyhow::Result<()> {
        ...
        let provider = match operant_providers::create_provider_with_url(
            provider_name,
            api_key,
            self.config
                .providers
