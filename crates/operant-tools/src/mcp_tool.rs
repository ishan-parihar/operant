//! Wraps a discovered MCP tool as a operant [`Tool`] so it is dispatched
...
/// A operant [`Tool`] backed by an MCP server tool.
///
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Operant's security model injects `approved: bool` into built-in tool
...
mod tests {
    ...
    // Operant's security model injects `approved: bool` into built-in tool args.
