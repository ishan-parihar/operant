...
use operant_config::policy::ToolOperation;
...
impl Tool for SessionsSendTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let chat_msg = operant_api::provider::ChatMessage::user(message);
...
impl Tool for SessionsCurrentTool {
    ...
    async fn execute(&self, _args: serde_json::Value) -> anyhow::Result<ToolResult> {
        let session_key = operant_api::TOOL_LOOP_SESSION_KEY
...
mod tests {
    use super::*;
    use operant_api::provider::ChatMessage;
    ...
    async fn sessions_current_returns_key_when_scoped() {
        ...
        let result = operant_api::TOOL_LOOP_SESSION_KEY
    ...
    async fn sessions_current_includes_name() {
        let tmp = TempDir::new().unwrap();
        let sqlite = operant_infra::session_sqlite::SqliteSessionBackend::new(tmp.path()).unwrap();
