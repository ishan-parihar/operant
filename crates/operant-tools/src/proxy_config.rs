    ...
    async fn handle_set(&self, args: &Value) -> anyhow::Result<ToolResult> {
        ...
        if let Some(scope_raw) = args.get("scope") {
            ...
            proxy.scope = Self::parse_scope(scope).ok_or_else(|| {
                anyhow::anyhow!("Invalid scope '{scope}'. Use environment|operant|services")
            })?;
        }
...
impl Tool for ProxyConfigTool {
    ...
    fn description(&self) -> &str {
        "Manage Operant proxy settings (scope: environment | operant | services), including runtime and process env application"
    }
    ...
    fn parameters_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "scope": {
                    "type": "string",
                    "description": "Proxy scope: environment | operant | services"
                },
                "http_proxy": {
                    "type": ["string", "null"],
