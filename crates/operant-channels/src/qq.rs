...
impl QQChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        operant_config::schema::build_channel_proxy_client("channel.qq", self.proxy_url.as_deref())
    }
...
allowed_users = ["user1"]
"#;
        let config: operant_config::schema::QQConfig = toml::from_str(toml_str).unwrap();
