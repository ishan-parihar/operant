...
// Force the compiler to recognize the Channel impl
const _: () = {
    let _ = <EmailChannel as Channel>::name;
};

pub use operant_config::scattered_types::EmailConfig;

type TokioTlsStream = TlsStream<TcpStream>;
type ImapSession = Session<tokio_util::compat::Compat<TokioTlsStream>>;
...
impl EmailChannel {
    ...
    fn extract_attachments(
    ...
    ) -> Vec<operant_api::media::MediaAttachment> {
        ...
        for part in parsed.attachments() {
            ...
            attachments.push(operant_api::media::MediaAttachment {
                file_name,
                data,
                mime_type: mime_str,
            });
    ...
    async fn connect_imap(&self) -> Result<ImapSession> {
        ...
        // Wrap with tokio_util::compat for futures-util AsyncRead/AsyncWrite compatibility
        ...
        // Login
        let session: ImapSession = client
    ...
    async fn fetch_unseen(&self, session: &mut ImapSession) -> Result<Vec<ParsedEmail>> {
        ...
        let uids: std::collections::HashSet<u32> = session.uid_search("UNSEEN").await?;
        if uids.is_empty() {
            return Ok(Vec::new());
        }
        ...
        for chunk in uids_vec.chunks(Self::MAX_FETCH_BATCH) {
            ...
            let messages: Vec<async_imap::types::Fetch> = messages.try_collect().await?;
    ...
    async fn run_session(&self, tx: &mpsc::Sender<ChannelMessage>) -> Result<()> {
        ...
        let mut session: ImapSession = self.connect_imap().await?;
...
struct ParsedEmail {
    ...
    content: String,
    timestamp: u64,
    attachments: Vec<operant_api::media::MediaAttachment>,
}
...
mod tests {
    ...
    fn email_config_deserialize_with_defaults() {
        ...
        assert_eq!(config.default_subject, "Operant Message"); // default
    }
