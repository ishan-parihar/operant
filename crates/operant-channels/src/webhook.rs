...
#[derive(Clone)]
struct WebhookState {
    tx: Arc<Mutex<tokio::sync::mpsc::Sender<ChannelMessage>>>,
...
impl WebhookChannel {
    ...
    fn http_client(&self) -> reqwest::Client {
        operant_config::schema::build_runtime_proxy_client("channel.webhook")
    }
...
async fn handle_webhook(
    ...
    if state.tx.lock().await.send(msg).await.is_err() {
                    ...
                    post(
                        |State(state): State<Arc<WebhookState>>,
                         ...
                         body: Bytes| async move {
                            ...
                            if let Some(ref secret) = state.secret {
                                ...
                                let valid = if let Some(sig) = signature {
                                        ...
                                        HmacSha256::new_from_slice(secret.as_bytes())
                                    {
                                        mac.update(&body);
                                        let expected =
                                            hex::decode(sig.trim_start_matches("sha256="))
                                                .unwrap_or_default();
                                        mac.verify_slice(&expected).is_ok()
                                    } else {
                                        false
                            ...
                            // Clone the sender before awaiting to avoid holding MutexGuard across await
                            let sender = state.tx.lock().await.clone();
            ...
            tokio::select! {
                result = listener.accept() => {
                    ...
                    tokio::spawn(async move {
                        // Create a new service instance from the make service
                        let svc = make_svc.call(()).await.unwrap();
                        let svc = hyper_util::service::TowerToHyperService::new(svc);
                        let io = hyper_util::rt::TokioIo::new(tcp_stream);
                        if let Err(e) = acceptor.serve_connection_with_upgrades(io, svc).await {
...
mod tests {
    ...
    fn incoming_payload_deserializes_all_fields() {
        let json = r#"{"sender": "operant_user", "content": "hello", "thread_id": "t1"}"#;
        let payload: IncomingWebhook = serde_json::from_str(json).unwrap();
        assert_eq!(payload.sender, "operant_user");
    ...
    fn outgoing_payload_serializes_content() {
        let payload = OutgoingWebhook {
            ...
            recipient: Some("operant_user".into()),
        };
        let json = serde_json::to_value(&payload).unwrap();
        assert_eq!(json["content"], "response");
        assert_eq!(json["thread_id"], "t1");
        assert_eq!(json["recipient"], "operant_user");
    }
