...
/// and forwards PRIVMSG messages to the `Operant` message bus.
...
impl Channel for IrcChannel {
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        Self::send_raw(&mut writer, &format!("USER {} 0 * :Operant", self.username)).await?;
...
mod tests {
    ...
    fn irc_config_serde_roundtrip() {
        use operant_config::schema::IrcConfig;
        ...
        assert_eq!(parsed.username.as_deref(), Some("operant"));
