...
- **Noted (minor)**: the runtime `for` loop consumes an iteration per retry (no refund like `OperantAgent::iteration_budget.refund()`) — immaterial at the live 90-iteration budget; a while-loop conversion was rejected as too risky (12 `continue` sites).
