...
| Phase 7 — hermes parity (tool planning, telemetry, eval) | 🔶 C1 ✅ + C4 ✅; C2/C3 open | C4 (`node_detail()` read-before-edit) shipped 87a97c71 with 5 tests. C1 (`operant-tool-planning` crate + `platform_toolsets` config + runtime `api_server` allow-list wiring) shipped 1dc42f4f with 9 tests. C2 = observability substance already shipped in Phase 2a (only the bridge observer helper remains — optional). C3 (eval harness) deferred until a real model endpoint is configured. |
