...
| Phase 7 — hermes parity (tool planning, telemetry, eval) | ✅ C1/C2/C4 + C3 core | C4 (`node_detail()` read-before-edit) 87a97c71 (5 tests). C1 (`operant-tool-planning` + `platform_toolsets` + runtime `api_server` allow-list) 1dc42f4f + 39815e5c (9 tests). C2: **verified already shipped** — PrometheusObserver implements the API-family Observer the runtime loop emits into (llm_request/tool_call/error/cache counters), `create_observer` wires `PrometheusObserver::shared()` for the `prometheus` backend; 82 feature-gated tests pass; no bridge needed (65b883e6 records the verdict). C3 core (`operant-eval`: task loader, subsequence/keyword verifier, reporter, 5 golden YAMLs, 11 tests) shipped 65b883e6; the endpoint-driven runner + `lint-checks.sh` step 5 remain deferred until a real model endpoint is configured. |
| Phase 9 — audit refresh | ✅ DONE | `docs/HERMES_VS_OPERANT_AUDIT_2026-08-03.md` supersedes the stale July audit (prompt caching + streaming scrubber now implemented; TDG→agentmemory + Obscura→igs migrations done). Gaps G1–G6 recorded; G5 (BUGS.md triage) done — doc re-verified 2026-08-03, only embedded-web dist stays open (Low). |
...
| Phase 10 — dead_code sweep | ✅ DONE (this iteration) | 38/38 core `#[allow(dead_code)]` → `#[expect(dead_code, reason)]` (21 serde tool-arg structs, 4 test structs, 13 purpose sites); 5 stale suppressions removed on live code (turn_finalizer footer, background_review NotificationMode, insights SessionRow.id, anthropic chat() stale unwrap_used expect, turn_context already_added → `cfg_attr(not(test), expect)` test seam). 0 unfulfilled under default + all-features; lib 0 warnings; 1249 core tests pass. Remaining crates (gateway 35, cli 114, channels 27, providers 13, tools 9, runtime 6, memory 4) are follow-on iterations. |
...
**Note:** the plan's original hotspot counts (cron/store 131, webauthn 90, …) were gross counts *including* test modules; the production-only reality was 65 unwraps + 40 expects. All eliminated: static-literal regex unwraps → `rx()` helpers (leak_detector, prompt_guard, loop_), lock-invariant + guarded-by-check sites → justified `expect`, data-dependent `tool_call_id` sites → graceful `if let`. prometheus.rs (~25 expects) is **resolved**: the `observability-prometheus` feature is wired to real deps (eb76a5a7) and `PrometheusObserver::new()` carries a function-level `#[expect(clippy::expect_used, reason = "invariant guaranteed by surrounding validation")]`; verified compiling with 82 feature-gated tests in Phase 7 C2.
...
3. Replace `#[allow(dead_code)]` on real-but-unwired code (learning-graph mutations, MCP infra) with `#[expect(dead_code, reason = "...")]` + a tracked TODO, or wire the code (see Phase 7). ✅ **core done** (38/38 → Phase 10); remaining crates (gateway 35, cli 114, channels 27, providers 13, tools 9, runtime 6, memory 4) are follow-on iterations.
