...
| Phase 5 — anyhow → typed errors | 🔶 **half done** | ✅ **operant-memory DONE** (2cfaa97e): typed `Error` + `MemoryContextExt` in `src/error.rs`, `MemoryResult<T> = Result<T, MemoryError>` seam in operant-api (Message/Io/Serde/Backend), anyhow removed incl. dev-deps, 6 error-seam tests, gate locale fix (LC_ALL=C comm). ⛔ **operant-gateway PENDING**: still `anyhow = "1.0"`, 11 `anyhow::Result` sites across 7 files (api_config, node_tool, ws_approval, ws, api, lib, tls). |
