...
- **Persistent Prime Kernel (opt-in)** — a stateful Python kernel whose variables survive across turns, a continual-harness store (session-local + global prompt/subagent lessons with snapshot rollback), and a tool bridge that lets one kernel cell call allowlisted tools in a loop (RLM-lite). See [docs/pk-kernel.md](docs/pk-kernel.md). Disabled by default; `git submodule update --init --recursive` + `[tools.prime_kernel] enabled = true` to adopt.
